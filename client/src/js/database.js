...
export const db = {
  ...
  async createProduct(values) {
    ...
    const unitsPerPackage = Math.max(1, toNumber(values.unitsPerPackage) || 1); const nearestExpiryDate = normalize(values.nearestExpiryDate);
  ...
  async updateProduct(productId, values) {
    ...
    if (duplicate) throw new Error(`هذا الباركود مستخدم بالفعل للمنتج: ${duplicate.name}`);
    const nearestExpiryDate = normalize(values.nearestExpiryDate ?? current.nearestExpiryDate);
    if (nearestExpiryDate && (!/^\d{4}-\d{2}-\d{2}$/.test(nearestExpiryDate) || nearestExpiryDate <= dateKey())) throw new Error("أدخل تاريخ انتهاء مستقبليًا صالحًا أو اترك الحقل فارغًا.");
  ...
  async completeSale({ items, discount, paidAmount, paymentMethod, paymentType = "نقدي", customerId = "", deliveryFee = 0, deliveryChargeType = "store", cashierShiftId = "", cashierId = "", cashierName = "" }) {
    if (!items.length) throw new Error("أضف منتجًا واحدًا على الأقل إلى السلة.");
    const database = await this.open(); const delivery = roundMoney(Math.max(0, toNumber(deliveryFee))); const deliveryType = deliveryChargeType === "customer" ? "customer" : "store"; const needsCustomer = paymentType === "آجل"; const transaction = database.transaction(["settings", "products", "productBatches", "sales", "saleItems", "stockMovements", "meta", "customers", "customerTransactions", "expenses"], "readwrite"); const settings = await requestAsPromise(transaction.objectStore("settings").get("app")); const pharmacyMode = settings?.businessType === "صيدلية"; const products = transaction.objectStore("products"); const batches = transaction.objectStore("productBatches"); const resolved = []; const today = dateKey();
    for (const line of items) { const product = await requestAsPromise(products.get(line.productId)); if (!product || product.isDeleted) throw new Error("أحد منتجات السلة لم يعد متاحًا."); if (!canSell(product.quantity, line.quantity)) throw new Error(`الكمية المتوفرة غير كافية للمنتج: ${product.name}`); const trackedBatches = (await requestAsPromise(batches.index("productId").getAll(product.id))).filter((batch) => toNumber(batch.remainingQuantity) > 0 && batch.expiryDate).sort((a, b) => String(a.expiryDate).localeCompare(String(b.expiryDate))); const availableBatches = pharmacyMode ? trackedBatches.filter((batch) => batch.expiryDate >= today) : trackedBatches; if (pharmacyMode && availableBatches.reduce((sum, batch) => sum + toNumber(batch.remainingQuantity), 0) < toNumber(line.quantity)) throw new Error(`لا توجد كميات صالحة كافية ضمن تشغيلات المنتج: ${product.name}`); resolved.push({ line, product, availableBatches }); }
    const totals = calculateSaleTotals(resolved.map(({ line, product }) => ({ productId: product.id, unitPrice: product.salePrice, quantity: line.quantity, discount: line.discount })), discount); const invoiceTotal = roundMoney(totals.total + (deliveryType === "customer" ? delivery : 0)); const isCredit = paymentType === "آجل"; const paid = isCredit ? 0 : Math.max(0, toNumber(paidAmount)); if (paid > invoiceTotal) throw new Error("المبلغ المدفوع لا يمكن أن يتجاوز إجمالي الفاتورة."); if (!isCredit && paid < invoiceTotal) throw new Error("المبلغ المدفوع أقل من الإجمالي النهائي."); const customer = needsCustomer ? await requestAsPromise(transaction.objectStore("customers").get(customerId)) : null; if (needsCustomer && (!customerId || !customer || customer.isDeleted)) throw new Error("اختر عميلًا نشطًا للبيع الآجل.");
    ...
    for (const { line, product, availableBatches } of resolved) { const calculatedLine = totals.lines.find((item) => item.productId === product.id) || totals.lines.find((item) => item.unitPrice === product.salePrice && item.quantity === line.quantity); const quantity = toNumber(line.quantity); const previousQuantity = toNumber(product.quantity); const newQuantity = previousQuantity - quantity; const unitCost = toNumber(product.purchasePrice); let remainingToAllocate = quantity; const batchAllocations = []; availableBatches.forEach((batch) => { const allocated = Math.min(remainingToAllocate, toNumber(batch.remainingQuantity)); if (allocated <= 0) return; const remainingQuantity = roundMoney(toNumber(batch.remainingQuantity) - allocated); batches.put({ ...batch, remainingQuantity, updatedAt: date }); batchAllocations.push({ batchId: batch.id, batchNumber: batch.batchNumber, expiryDate: batch.expiryDate, quantity: allocated }); remainingToAllocate = roundMoney(remainingToAllocate - allocated); }); const nextExpiry = availableBatches.filter((batch) => toNumber(batch.remainingQuantity) - (batchAllocations.find((item) => item.batchId === batch.id)?.quantity || 0) > 0).map((batch) => batch.expiryDate).filter(Boolean).sort()[0] || ""; products.put({ ...product, quantity: newQuantity, nearestExpiryDate: nextExpiry || (availableBatches.length ? "" : product.nearestExpiryDate || ""), updatedAt: date }); transaction.objectStore("saleItems").add({ id: uid("sale-item"), saleId: sale.id, productId: product.id, productName: product.name, unit: product.unit, quantity, unitPrice: toNumber(product.salePrice), unitCost, discount: calculatedLine?.discount || 0, discountInput: line.discount || "", total: calculatedLine?.total ?? roundMoney(product.salePrice * quantity), costTotal: roundMoney(unitCost * quantity), batchAllocations, returnedQuantity: 0 }); createStockMovement(transaction.objectStore("stockMovements"), { productId: product.id, type: "SALE", quantity: -quantity, previousQuantity, newQuantity, date, note: `بيع ضمن الفاتورة ${sale.invoiceNumber}`, referenceType: "SALE", referenceId: sale.id }); }
  ...
  async createPurchase({ supplierId = "", items, notes = "", paymentType = "نقدي", paidAmount = "", paymentMethod = "نقدي" }) {
    ...
    for (const line of items) {
      ...
      const batchNumber = normalize(line.batchNumber); const expiryDate = normalize(line.expiryDate);
      if (pharmacyMode && !batchNumber) throw new Error(`أدخل رقم التشغيلة للمنتج: ${product.name}`);
      ...
      resolved.push({ product, ...packaging, packageUnit: normalize(line.packageUnit || product.purchasePackageUnit || "حبة") || "حبة", salePrice, batchNumber, expiryDate });
    }
    ...
    for (const { product, quantity, unitCost, total: itemTotal, packageQuantity, unitsPerPackage, packageCost, packageUnit, salePrice, batchNumber, expiryDate } of resolved) {
      ...
      const batchId = expiryDate ? uid("batch") : "";
      transaction.objectStore("purchaseItems").add({ id: uid("purchase-item"), purchaseId: purchase.id, productId: product.id, productName: product.name, unit: product.unit, quantity, unitCost, total: itemTotal, packageUnit, packageQuantity, unitsPerPackage, packageCost, salePrice, batchNumber, expiryDate, batchId, returnedQuantity: 0 });
      if (expiryDate) batches.add({ id: batchId, productId: product.id, productName: product.name, purchaseId: purchase.id, batchNumber, expiryDate, quantity, remainingQuantity: quantity, unitCost, createdAt: date, updatedAt: date });
  ...
  async startCashierShift({ accountId, accountName, receivedCash }) {
    ...
    const previous = all.filter((item) => item.status === "CLOSED" && !item.receivedByShiftId && item.date === shift.date).sort((a, b) => new Date(b.closedAt) - new Date(a.closedAt))[0]; if (previous) { shifts.put({ ...previous, receivedByShiftId: shift.id, receivedByAccountId: accountId, receivedAt: startedAt }); shift.receivedFromShiftId = previous.id; }
