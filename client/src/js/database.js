...
export const db = {
  ...
  async completeSale({ items, discount, paidAmount, paymentMethod, paymentType = "نقدي", customerId = "", dueDate = "", deliveryFee = 0, deliveryChargeType = "store", cashierShiftId = "", cashierId = "", cashierName = "", sellerRole = "" }) {
    ...
    for (const service of serviceSaleLines) { const calculatedService = totals.lines.find((item) => item.productId === service.productId); transaction.objectStore("saleItems").add({ id: uid("sale-item"), saleId: sale.id, productId: "", productName: service.name, unit: "خدمة", quantity: 1, unitPrice: service.unitPrice, unitCost: service.unitPrice, discount: calculatedService?.discount || 0, discountInput: service.discount || "", total: calculatedService?.total ?? service.unitPrice, costTotal: service.unitPrice, soldAsPackage: false, packageUnit: "", packageQuantity: 0, unitsPerPackage: 1, batchAllocations: [], returnedQuantity: 0, serviceType: service.serviceType }); }
    /* نقد مقابل تحويل: الزبون يحوّل حوالة ويستلم نقدًا — يُسجل خروج النقد من الصندوق تلقائيًا لتبقى الأرصدة صادقة. */
    const cashForTransferTotal = roundMoney(serviceSaleLines.filter((service) => service.serviceType === "cash-transfer").reduce((sum, service) => sum + (totals.lines.find((item) => item.productId === service.productId)?.total ?? service.unitPrice), 0));
    if (cashForTransferTotal > 0) transaction.objectStore("cashMovements").add({ id: uid("cash-movement"), type: "WITHDRAWAL", amount: cashForTransferTotal, date: dateKey(date), notes: `نقد مقابل تحويل — فاتورة ${sale.invoiceNumber}`, referenceType: "SALE", referenceId: sale.id, createdAt: date });
