...
export const db = {
  ...
  async completeSale({ items, discount, paidAmount, paymentMethod, paymentType = "نقدي", customerId = "", deliveryFee = 0, deliveryChargeType = "store", cashierShiftId = "", cashierId = "", cashierName = "", sellerRole = "" }) {
    ...
    const totals = calculateSaleTotals(resolved.map(({ line, product }) => ({ productId: product.id, unitPrice: product.salePrice, quantity: line.quantity, discount: line.discount })), discount); const isCashierSeller = sellerRole === "cashier" || Boolean(cashierId); const cashierDiscountCap = roundMoney(totals.subtotal * 0.1); if (isCashierSeller && totals.discount > cashierDiscountCap) throw new Error(`أقصى خصم للكاشير هو 10% من سعر البيع (${cashierDiscountCap}).`); const invoiceTotal = roundMoney(totals.total + (deliveryType === "customer" ? delivery : 0)); const isCredit = paymentType === "آجل"; const paid = isCredit ? 0 : Math.max(0, toNumber(paidAmount)); if (paid > invoiceTotal) throw new Error("المبلغ المدفوع لا يمكن أن يتجاوز إجمالي الفاتورة."); if (!isCredit && paid < invoiceTotal) throw new Error("المبلغ المدفوع أقل من الإجمالي النهائي."); const customer = needsCustomer ? await requestAsPromise(transaction.objectStore("customers").get(customerId)) : null; if (needsCustomer && (!customerId || !customer || customer.isDeleted)) throw new Error("اختر عميلًا نشطًا للبيع الآجل.");
