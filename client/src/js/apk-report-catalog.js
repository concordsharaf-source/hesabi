...
export function getApkReportRows(type, state, helpers) {
  ...
  if (type === "accountsTotal") {
    ...
    return [["نوع الحساب", "عدد الحسابات", "إجمالي الرصيد", "أرصدة مدينة", "أرصدة دائنة"], ["العملاء", String(customers.length), money(helpers, customerBalance), money(helpers, customers.filter((item) => n(item.balance) > 0).reduce((total, item) => total + n(item.balance), 0)), money(helpers, customers.filter((item) => n(item.balance) < 0).reduce((total, item) => total + Math.abs(n(item.balance)), 0))], ["الموردون", String(suppliers.length), money(helpers, supplierBalance), money(helpers, suppliers.filter((item) => n(item.balance) < 0).reduce((total, item) => total + Math.abs(n(item.balance)), 0)), money(helpers, suppliers.filter((item) => n(item.balance) > 0).reduce((total, item) => total + n(item.balance), 0))], ["الإجمالي", String(customers.length + suppliers.length), money(helpers, customerBalance + supplierBalance), "", ""]];
  }

  if (type === "accountBalance") {
    return [["نوع الحساب", "اسم الحساب", "الهاتف", "العنوان", "الرصيد", "حالة الحساب"], ...(state.customers || []).map((item) => ["عميل", text(item.name), text(item.phone), text(item.address), money(helpers, item.balance), n(item.balance) > 0 ? "عليه رصيد مستحق" : n(item.balance) < 0 ? "له رصيد دائن" : "متزن"]), ...(state.suppliers || []).map((item) => ["مورد", text(item.name), text(item.phone), text(item.address), money(helpers, item.balance), n(item.balance) > 0 ? "مستحق له" : n(item.balance) < 0 ? "عليه رصيد" : "متزن"] )];
  }
