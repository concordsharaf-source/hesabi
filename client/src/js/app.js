...
const categorySelectMarkup = (value = "") => `<label>الصنف<input name="category" list="product-category-list" dir="rtl" value="${escapeHtml(value)}" placeholder="اختر أو اكتب صنفًا جديدًا" /><datalist id="product-category-list">${categoryOptions(value).map((category) => `<option value="${escapeHtml(category)}">`).join("")}</datalist></label>`;
let root;
...
function confirmImportReplacement(count) { return new Promise((resolve) => { const overlay = openDialog(`<div class="dialog__head"><div><span class="eyebrow">تأكيد الاستيراد</span><h2>بيانات موجودة مسبقًا</h2><p class="dialog__subtext">وجدنا ${count} منتجات موجودة ببيانات مختلفة. هل تريد استبدال بياناتها بالبيانات الموجودة في الملف؟</p></div><button class="icon-button" data-dialog-close aria-label="إغلاق">${icon("close", 20)}</button></div><div class="import-confirm-choice"><div><strong>موافق</strong><span>استبدال بيانات المنتجات الموجودة ثم إضافة الجديدة.</span></div><div><strong>تخطي</strong><span>الحفاظ على بيانات المنتجات الموجودة وإضافة الجديدة فقط.</span></div></div><div class="dialog__actions"><button type="button" class="button button--secondary" data-import-skip>تخطي الموجود</button><button type="button" class="button button--primary" data-import-replace>موافق واستبدال</button></div>`); const finish = (value) => { closeDialog(); resolve(value); }; overlay.querySelector("[data-import-replace]").addEventListener("click", () => finish(true)); overlay.querySelector("[data-import-skip]").addEventListener("click", () => finish(false)); overlay.querySelector("[data-dialog-close]").addEventListener("click", () => finish(false)); }); }
