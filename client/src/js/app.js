...
async function shareInvoice(invoice) {
  try {
    ...
    const result = await shareOrDownloadInvoicePdf({ invoice: invoiceWithCustomer, customer, storeName: storeDisplayName(), storeInfo: state.settings, logoDataUrl: storeLogoDataUrl(), formatMoney: money, formatAmount: amount, formatDateTime: dateTime, paymentLabel: paymentChannelLabel(invoiceWithCustomer), filename: `${invoice.invoiceNumber}.pdf`, title: `فاتورة ${invoice.invoiceNumber}` });
...
async function openCustomerAccountDialog(customerId) {
  ...
  overlay.querySelector("#share-customer-account").addEventListener("click", async () => { try { const result = await shareOrDownloadCustomerAccountPdf({ account: printAccount, storeName: storeDisplayName(), storeInfo: state.settings, logoDataUrl: storeLogoDataUrl(), formatMoney: money, formatDateTime: dateTime, filename: `كشف-حساب-${account.customer.name}.pdf`, title: `كشف حساب ${account.customer.name}` }); showToast(result === "shared" ? "تمت مشاركة كشف الحساب PDF" : "تم تنزيل كشف الحساب PDF للمشاركة"); } catch (error) { if (error?.name !== "AbortError") showToast("تعذر إنشاء PDF لكشف الحساب.", "error"); } });
}
...
async function sharePurchasePdf(purchase) {
  ...
  const result = await shareOrDownloadPurchaseInvoicePdf({ purchase: purchaseForPdf, supplier, storeName: storeDisplayName(), storeInfo: state.settings, logoDataUrl: storeLogoDataUrl(), formatMoney: money, formatAmount: amount, formatDateTime: dateTime, filename: `${purchase.invoiceNumber}.pdf`, title: `فاتورة شراء ${purchase.invoiceNumber}` });
