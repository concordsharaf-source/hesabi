...
const state = { view: "dashboard", lastStableView: "dashboard", showSetupHome: false, showStoreChooser: false, pendingStoreId: "", settings: null, accounts: [], currentUser: null, activeCashierShift: null, cashierShifts: [], cashierSalarySummaries: [], cashierShiftStatistics: [], vault: null, products: [], productSuppliers: {}, sales: [], suppliers: [], supplierPayments: [], customers: [], customerPayments: [], purchases: [], expenses: [], stockMovements: [], cashMovements: [], transferVaultDeposits: [], cashbox: null, dashboard: null, analytics: null, periodicInventories: [], periodicInventorySummary: null, auditCycle: "monthly", auditFrom: "", auditTo: "", cart: [], productQuery: "", productCategory: "الكل", inventoryCategory: "الكل", saleQuery: "", invoiceQuery: "", supplierQuery: "", customerQuery: "", paymentQuery: "", paymentFrom: "", paymentTo: "", supplierPaymentQuery: "", supplierPaymentFrom: "", supplierPaymentTo: "", cashFrom: "", cashTo: "", debtQuery: "", debtSort: "highest", expenseQuery: "", expenseFrom: "", expenseTo: "", reportFrom: "", reportTo: "", scanner: null, cartDiscount: "", cloud: { user: null, backups: [], loading: false, busy: "", error: "", identity: null, pairing: null, pairRequests: [], syncStatus: "local" } };
const STORE_REGISTRY_KEY = "hesabi-store-registry-v1";
function readStoreRegistry() { try { const value = JSON.parse(localStorage.getItem(STORE_REGISTRY_KEY) || "[]"); return Array.isArray(value) ? value : []; } catch { return []; } }
function writeStoreRegistry(entries) { try { localStorage.setItem(STORE_REGISTRY_KEY, JSON.stringify(entries.slice(0, 30))); } catch { /* التخزين المحلي قد يكون ممتلئًا. */ } }
function registerCurrentStore(settings) { if (!settings?.setupCompleted) return; const id = settings.storeId || "default"; const entries = readStoreRegistry().filter((entry) => entry.id !== id); entries.unshift({ id, name: settings.storeName || "متجر جديد", businessType: settings.businessType || "متجر عام", updatedAt: new Date().toISOString() }); writeStoreRegistry(entries); }
function newStoreId() { return `store-${randomId()}`; }
function storeChooserMarkup() { const stores = readStoreRegistry(); return `<main class="setup-page login-page"><section class="setup-art"><div class="setup-art__brand"><img src="${markImage}" alt="شعار حسابي" /><span class="brand-wordmark">حسابي</span><small>إدارة متاجر مستقلة</small></div><div class="setup-art__copy"><p class="eyebrow">الرئيسية</p><h1>اختر المتجر<br />الذي تريد فتحه.</h1><p>لكل متجر منتجات وفواتير وحسابات ونسخ سحابية مستقلة تمامًا.</p></div></section><section class="setup-form-wrap"><div class="setup-sheet"><div class="setup-sheet__brand"><img src="${markImage}" alt="شعار حسابي" /><div><strong>متاجري</strong><span>بيانات منفصلة لكل نشاط</span></div></div><div class="setup-form"><span class="eyebrow">اختيار المتجر</span><h2>ابدأ من المكان الصحيح</h2><p>اختر متجرًا موجودًا أو أنشئ ملفًا جديدًا للصيدلية أو البقالة دون خلط البيانات.</p><div class="store-chooser-list">${stores.map((store) => `<button type="button" class="account-hint" data-action="select-store" data-store-id="${escapeHtml(store.id)}"><span>${escapeHtml(store.name)}</span><small>${escapeHtml(store.businessType)}</small></button>`).join("") || `<div class="inline-empty">لا توجد متاجر مسجلة بعد.</div>`}</div><button type="button" class="button button--primary button--wide" data-action="new-store">إضافة متجر جديد ${icon("plus", 18)}</button><div class="store-home-actions"><label class="button button--secondary button--wide" for="home-restore-file">استعادة نسخة محلية وفتح متجرها ${icon("restore", 18)}<input id="home-restore-file" type="file" accept="application/json,.json" hidden /></label><button type="button" class="button button--secondary button--wide" data-action="cloud-restore-start">دخول بإيميل النسخ الاحتياطي ${icon("restore", 18)}</button><button type="button" class="button button--secondary button--wide" data-action="assistant-login">دخول جهاز مساعد ${icon("arrow", 18)}</button></div><button type="button" class="button button--secondary button--wide" data-action="return-current-store">العودة إلى متجر الدخول الحالي</button></div></div></section></main>`; }
...
async function refresh() {
  [state.products, state.productSuppliers, state.sales, state.suppliers, state.supplierPayments, state.customers, state.customerPayments, state.purchases, state.expenses, state.stockMovements, state.cashMovements, state.transferVaultDeposits, state.cashbox, state.dashboard, state.cashierShifts, state.cashierSalarySummaries, state.cashierShiftStatistics, state.vault, state.periodicInventories] = await Promise.all([db.listProducts(), db.listProductSupplierLinks(), db.listSales(), db.listSuppliers(), db.listSupplierPayments(), db.listCustomers(), db.listCustomerPayments(), db.listPurchases(), db.listExpenses(), db.listStockMovements(), db.listCashMovements({ from: state.cashFrom, to: state.cashTo }), db.listTransferVaultDeposits(), db.getCashbox({ from: state.cashFrom, to: state.cashTo }), db.getDashboard(), db.listCashierShifts({ date: "" }), db.listCashierSalarySummaries(), db.listCashierShiftStatistics({ from: state.cashFrom, to: state.cashTo }), db.getVault({ from: state.cashFrom, to: state.cashTo }), db.listPeriodicInventories()]);
...
function inventoryMarkup() {
  ...
  <section class="inventory-summary"><div><span>إجمالي قيمة المخزون</span><strong>${money(state.dashboard.inventoryValue)}</strong></div><div><span>عدد المنتجات</span><strong>${amount(state.products.length)} منتج</strong></div><div><span>منخفض أو نافد</span><strong>${amount(state.dashboard.lowStock.length)} منتج</strong></div></section><div class="category-toolbar"><strong>أصناف المخزون (${amount(products.length)})</strong><div class="category-chips">${categoryButtons(state.inventoryCategory, "inventory-category")}</div></div>
...
function periodicInventoryMarkup() {
  ...
  return `${topbarMarkup("الجرد المحاسبي الدوري", "لقطة محاسبية للفترة المختارة؛ الاعتماد لا يغيّر كميات المنتجات أو الصندوق أو الخزنة.", `<button class="button button--primary" data-action="save-periodic-inventory">${icon("check", 17)}<span>اعتماد الجرد</span></button>`)}
  ...
  <section class="panel periodic-inventory-history"><div class="panel__head"><div><span class="eyebrow">سجل غير قابل للتعديل</span><h2>لقطات الجرد المحفوظة</h2></div><small>${amount(history.length)} لقطة</small></div>${prior ? `<p class="periodic-inventory-history__note">سيُحفظ الاعتماد الجديد مع مقارنة بآخر جرد ${periodicInventoryCycleLabel(state.auditCycle)}.</p>` : ""}<div class="periodic-inventory-history__list">${historyMarkup}</div></section></div>`;
}
...
function expensesMarkup() {
  ...
  const expenseRows = (items, periodType) => items.length ? items.map((expense) => `<article class="entity-row"><div class="entity-row__icon entity-row__icon--expense">${icon(periodType === "monthly" ? "calendar" : "wallet", 20)}</div><div class="entity-row__main"><strong>${escapeHtml(expense.category)}</strong><small>${escapeHtml(expense.description || "بلا وصف")} · ${expense.date}${expense.cashierSalaryAdvance ? ` · سلفة ${escapeHtml(expense.cashierName || "كاشير")}` : ""}${periodType === "monthly" ? " · يوزع على أيام الشهر" : ""}</small></div><strong class="entity-row__amount">${money(expense.amount)}</strong><div class="entity-row__actions">${expense.cashierSalaryAdvance ? "" : `<button class="icon-button" aria-label="تعديل المصروف" data-action="edit-expense" data-id="${expense.id}">${icon("edit", 18)}</button>`}<button class="icon-button icon-button--danger" aria-label="حذف المصروف" data-action="delete-expense" data-id="${expense.id}">${icon("trash", 18)}</button></div></article>`).join("") : `<div class="inline-empty">لا توجد مصروفات ${periodType === "monthly" ? "شهرية" : "يومية"} ضمن الفترة.</div>`;
  return `${topbarMarkup("المصروفات", "سجّل المصروفات لتظهر في صافي الربح والتقارير.", `<div class="topbar__actions"><button class="button button--secondary" data-action="new-cashier-salary-advance">${icon("wallet", 17)}<span>سلفة كاشير</span></button><button class="button button--primary" data-action="new-expense">${icon("plus", 18)}<span>إضافة مصروف</span></button></div>`)}
...
function accountsMarkup() {
  ...
  const salaryByCashier = new Map((state.cashierSalarySummaries || []).map((summary) => [summary.accountId, summary]));
  ...
  <section class="panel account-list"><div class="panel__head"><div><span class="eyebrow">فريق المتجر</span><h2>الحسابات المحلية</h2></div><small>الأدمن: كامل الصلاحيات · الكاشير: المبيعات والفواتير فقط</small></div>${accounts.map((account) => { const salary = salaryByCashier.get(account.id); return `<article class="account-row"><div class="account-row__icon">${icon("users", 20)}</div><div class="account-row__main"><strong>${escapeHtml(account.name)}</strong><small dir="ltr">${escapeHtml(account.username)}</small>${account.role === "cashier" ? `<small class="account-salary-note">راتب الشهر ${money(salary?.monthlySalary || 0)} · السلف ${money(salary?.advances || 0)} · خصم العجز ${money(salary?.shortageDeductions || 0)} · المتبقي ${money(salary?.remainingSalary || 0)}</small>` : ""}</div><span class="account-badge account-badge--${account.role}">${roleLabel(account.role)}</span><span class="status status--${account.isActive ? "available" : "empty"}">${account.isActive ? "نشط" : "موقوف"}</span><div class="entity-row__actions"><button class="icon-button" data-action="reset-account-pin" data-id="${account.id}" aria-label="إعادة تعيين رمز دخول ${escapeHtml(account.name)}">${icon("key", 18)}</button><button class="icon-button" data-action="open-account" data-id="${account.id}" aria-label="تعديل ${escapeHtml(account.name)}">${icon("dots", 18)}</button>${account.role === "cashier" && account.isActive ? `<button class="icon-button icon-button--danger" data-action="delete-cashier-account" data-id="${account.id}" aria-label="حذف الكاشير ${escapeHtml(account.name)}">${icon("trash", 18)}</button>` : ""}</div></article>`; }).join("")}</section>`;
}
...
async function activateStore(storeId) { try { await signOutCloudBackupUser(); } catch { /* لا نمنع تبديل المتجر عند عدم وجود جلسة سحابية. */ } state.cloud.user = null; state.cloud.identity = null; state.showStoreChooser = false; state.showSetupHome = false; state.pendingStoreId = ""; await db.selectStore(storeId); state.settings = await db.getSettings(); state.accounts = await db.listAccounts(); state.currentUser = state.settings?.setupCompleted ? await db.getPersistentSession() : null; if (state.settings?.setupCompleted) await refresh(); registerCurrentStore(state.settings); render(); }

function renderApplication() {
  updateStoreIcon();
  if (state.showStoreChooser) { root.innerHTML = storeChooserMarkup(); bindEvents(); return; }
    ...
    try {
      ...
      adminForm.addEventListener("submit", async (adminEvent) => {
        ...
        try {
          ...
          const backupSettings = payload.stores.settings.find((item) => item.id === "app") || {};
          await db.selectStore(backupSettings.storeId || newStoreId());
...
function bindEvents() {
  ...
  root.querySelector("[data-action=assistant-login]")?.addEventListener("click", openAssistantEntryDialog);
...
async function handleSetup(event) {
  ...
  if (state.pendingStoreId) { await db.selectStore(state.pendingStoreId); state.settings = await db.getSettings(); }
  await db.saveSettings(values);
  state.showSetupHome = false; state.showStoreChooser = false; registerCurrentStore({ ...state.settings, ...values, storeId: state.pendingStoreId || state.settings?.storeId || "default", setupCompleted: true }); state.pendingStoreId = "";
  state.settings = await db.getSettings();
  await refresh();
  render();
...
async function handleActionUnsafe(event) {
  ...
  if (action === "setup-home") { try { await signOutCloudBackupUser(); } catch { /* لا توجد جلسة سحابية أو تعذر فصلها. */ } state.cloud.user = null; state.cloud.identity = null; state.showStoreChooser = true; state.showSetupHome = false; render(); return; }
  if (action === "new-store") { state.pendingStoreId = newStoreId(); state.showStoreChooser = false; state.showSetupHome = true; state.settings = null; render(); return; }
  if (action === "return-current-store") { state.showStoreChooser = false; state.showSetupHome = false; render(); return; }
  if (action === "select-store") { await activateStore(event.currentTarget.dataset.storeId); return; }
...
function openAccountDialog(account = null) {
  ...
  roleSelect.closest("label").insertAdjacentHTML("afterend", `<label id="account-salary-field">الراتب الشهري<input name="monthlySalary" type="number" min="0" step="0.01" inputmode="decimal" value="${escapeHtml(account?.monthlySalary ?? "")}" placeholder="0" /><small class="field-hint">يستخدم لسلف الكاشير وخصومات راتبه فقط.</small></label>`);
  const salaryField = accountForm.querySelector("#account-salary-field");
  const syncSalaryField = () => { const cashier = roleSelect.value === "cashier"; salaryField.hidden = !cashier; salaryField.querySelector("input").disabled = !cashier; };
...
async function restoreBackupFromFile(event) { const file = event.currentTarget.files?.[0]; if (!file) return; try { const parsed = JSON.parse(await file.text()); db.validateBackup(parsed); if (!window.confirm("ستستبدل الاستعادة كل بيانات هذا الجهاز بالنسخة المختارة. ستفتح النسخة داخل متجرها المرتبط. هل تريد المتابعة؟")) { event.currentTarget.value = ""; return; } const backupSettings = parsed.stores.settings.find((item) => item.id === "app") || {}; await db.selectStore(backupSettings.storeId || newStoreId()); await db.restoreBackup(parsed); await db.clearPersistentSession(); state.settings = await db.getSettings(); state.accounts = await db.listAccounts(); state.currentUser = null; state.cart = []; state.cartDiscount = ""; state.view = "sales"; await refresh(); render(); showToast("تمت استعادة النسخة الاحتياطية بنجاح"); } catch (error) { showToast(error.message || "تعذر استعادة ملف النسخة الاحتياطية.", "error"); } finally { event.currentTarget.value = ""; } }
...
async function shareInvoice(invoice) {
  try {
    ...
    const result = await shareOrDownloadInvoicePdf({ invoice: invoiceWithCustomer, customer, storeName: storeDisplayName(), logoDataUrl: storeLogoDataUrl(), formatMoney: money, formatAmount: amount, formatDateTime: dateTime, paymentLabel: paymentChannelLabel(invoiceWithCustomer), filename: `${invoice.invoiceNumber}.pdf`, title: `فاتورة ${invoice.invoiceNumber}` });
...
async function openCustomerAccountDialog(customerId) {
  ...
  overlay.querySelector("#share-customer-account").addEventListener("click", async () => { try { const result = await shareOrDownloadCustomerAccountPdf({ account: printAccount, storeName: storeDisplayName(), logoDataUrl: storeLogoDataUrl(), formatMoney: money, formatDateTime: dateTime, filename: `كشف-حساب-${account.customer.name}.pdf`, title: `كشف حساب ${account.customer.name}` }); showToast(result === "shared" ? "تمت مشاركة كشف الحساب PDF" : "تم تنزيل كشف الحساب PDF للمشاركة"); } catch (error) { if (error?.name !== "AbortError") showToast("تعذر إنشاء PDF لكشف الحساب.", "error"); } });
}
...
function openCashierSalaryAdvanceDialog() {
  const cashiers = state.accounts.filter((account) => account.role === "cashier" && account.isActive);
  if (!cashiers.length) { showToast("أضف حساب كاشير نشطًا قبل تسجيل السلفة.", "error"); return; }
  const options = cashiers.map((cashier) => `<option value="${cashier.id}">${escapeHtml(cashier.name)}</option>`).join("");
  const overlay = openDialog(`<div class="dialog__head"><div><span class="eyebrow">خصم من الراتب</span><h2>سلفة كاشير</h2><p class="dialog__subtext">تسجل كمصروف نقدي، وتخصم من راتب الكاشير المتبقي في شهر التاريخ المحدد.</p></div><button class="icon-button" data-dialog-close aria-label="إغلاق">${icon("close", 20)}</button></div><form id="cashier-salary-advance-form" class="form-grid"><label>الكاشير<select name="cashierId">${options}</select></label><label>المبلغ<input name="amount" type="number" inputmode="decimal" min="0.01" step="0.01" required autofocus /></label><label>تاريخ السلفة<input name="date" type="date" value="${dateKey()}" required /></label><label class="form-full">الوصف<input name="description" dir="rtl" placeholder="مثال: سلفة من راتب الشهر" /></label><p id="cashier-salary-advance-note" class="form-full scanner-session-note"></p><label class="form-full">ملاحظات<textarea name="notes" dir="rtl"></textarea></label><div class="dialog__actions form-full"><button class="button button--secondary" type="button" data-dialog-close>إلغاء</button><button class="button button--primary" type="submit">تسجيل السلفة ${icon("check", 17)}</button></div></form>`);
  overlay.querySelectorAll("[data-dialog-close]").forEach((button) => button.addEventListener("click", closeDialog));
  ...
  const syncAdvanceNote = () => { const month = String(form.date.value || dateKey()).slice(0, 7); const summary = (state.cashierSalarySummaries || []).find((item) => item.accountId === form.cashierId.value && item.month === month); const cashier = cashiers.find((item) => item.id === form.cashierId.value); const salary = toNumber(summary?.monthlySalary ?? cashier?.monthlySalary); const advances = toNumber(summary?.advances); const shortageDeductions = toNumber(summary?.shortageDeductions); const remaining = summary ? toNumber(summary.remainingSalary) : roundMoney(salary - advances - shortageDeductions); note.textContent = salary > 0 ? `راتب ${cashier?.name || "الكاشير"}: ${money(salary)} · سلف الشهر: ${money(advances)} · خصم العجز: ${money(shortageDeductions)} · المتبقي: ${money(remaining)}` : "لم يُسجل راتب شهري لهذا الكاشير؛ عدّل بيانات حسابه أولًا."; };
  form.cashierId.addEventListener("change", syncAdvanceNote); form.date.addEventListener("change", syncAdvanceNote); syncAdvanceNote();
  form.addEventListener("submit", async (event) => { event.preventDefault(); try { const values = Object.fromEntries(new FormData(form)); await db.createExpense({ ...values, cashierSalaryAdvance: true, periodType: "daily" }); await refresh(); closeDialog(); render(); showToast("تم تسجيل سلفة الكاشير وخصمها من راتبه."); } catch (error) { showToast(error.message || "تعذر تسجيل سلفة الكاشير.", "error"); } });
}
...
async function openPurchaseDialog(purchaseIdOrDraft = null) {
  ...
  const makePurchaseLine = (product, values = {}) => {
    ...
    return { productId: product.id, productName: product.name, unit: product.unit || businessProfile().defaultUnit, packageUnit, salePrice: toNumber(salePrice), batchNumber: values.batchNumber || "", productionDate: values.productionDate || "", expiryDate: values.expiryDate || "", ...values, ...calculatePackagePurchase({ packageQuantity, unitsPerPackage, packageCost }) };
  };
