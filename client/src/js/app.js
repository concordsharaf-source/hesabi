...
async function downloadReportPdf() { try { const file = await createReportPdfFile({ rows: reportExportRows(), storeName: storeDisplayName(), storeInfo: state.settings, logoDataUrl: storeLogoDataUrl(), from: state.reportFrom || "بداية السجل", to: state.reportTo || "اليوم", filename: `hesabi-report-${dateKey()}.pdf` }); downloadGeneratedFile(file); showToast("تم تصدير تقرير PDF"); } catch (error) { showToast(error.message || "تعذر إنشاء تقرير PDF.", "error"); } }
