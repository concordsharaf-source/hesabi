...
async function refresh() {
  [state.products, state.productSuppliers, state.sales, state.suppliers, state.supplierPayments, state.customers, state.customerPayments, state.purchases, state.expenses, state.stockMovements, state.cashMovements, state.cashbox, state.dashboard, state.cashierShifts, state.cashierSalarySummaries] = await Promise.all([db.listProducts(), db.listProductSupplierLinks(), db.listSales(), db.listSuppliers(), db.listSupplierPayments(), db.listCustomers(), db.listCustomerPayments(), db.listPurchases(), db.listExpenses(), db.listStockMovements(), db.listCashMovements({ from: state.cashFrom, to: state.cashTo }), db.getCashbox({ from: state.cashFrom, to: state.cashTo }), db.getDashboard(), db.listCashierShifts(), db.listCashierSalarySummaries()]);
...
function cashierSalarySummaryMarkup() {
  ...
  return `<section class="panel cashier-salary-summary"><div class="panel__head"><div><span class="eyebrow">رواتب الشهر الحالي</span><h2>راتب الكاشير وسلفه</h2></div><small>${amount(summaries.length)} كاشير</small></div><p class="panel__subtext">تُسجل السلفة كمصروف نقدي وتخصم من المتبقي من راتب الكاشير لهذا الشهر.</p>${summaries.length ? `<div class="cashier-salary-list">${summaries.map((summary) => `<article class="cashier-salary-row"><div><strong>${escapeHtml(summary.accountName)}</strong><small>شهر ${escapeHtml(summary.month)}</small></div><div><small>الراتب</small><strong>${money(summary.monthlySalary)}</strong></div><div><small>السلف</small><strong class="${toNumber(summary.advances) ? "is-negative" : ""}">${money(summary.advances)}</strong></div><div><small>المتبقي</small><strong class="${toNumber(summary.remainingSalary) < 0 ? "is-negative" : ""}">${money(summary.remainingSalary)}</strong></div></article>`).join("")}</div>` : `<div class="inline-empty">لا توجد حسابات كاشير لعرض الرواتب.</div>`}</section>`;
}
...
function renderApplication() {
  ...
  if (state.view === "cashbox" && isAdmin(state.currentUser)) root.querySelector(".workspace")?.insertAdjacentHTML("beforeend", `${cashierShiftSummaryMarkup()}${cashierSalarySummaryMarkup()}`);
  bindEvents();
