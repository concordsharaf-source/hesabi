  ...
  try {
    ...
    for (const [index, { record, existing, changed }] of matches.entries()) {
      ...
      try {
        ...
        else { await db.createProduct({ name: record.name || `صنف ${record.barcode}`, barcode: record.barcode, internalCode: record.internalCode, purchasePrice: record.purchasePrice, salePrice: record.salePrice, quantity: record.quantity, unit: record.unit, category: record.category || "أخرى", minimumStock: 0 }); created += 1; }
