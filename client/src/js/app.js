...
function expensesMarkup() {
  ...
  const searchable = (expense) => !query || [expense.category, expense.description, expense.notes, expense.cashierName].some((value) => String(value || "").toLocaleLowerCase("ar").includes(query));
  const matches = state.expenses.filter((expense) => searchable(expense) && expense.date >= expenseFrom && expense.date <= expenseTo);
  ...
  const monthlyExpenses = [...matches.filter((expense) => expense.periodType === "monthly"), ...monthlySalaryExpenses].sort((a, b) => new Date(b.date) - new Date(a.date));
  ...
  const expenseRows = (items, periodType) => items.length ? items.map((expense) => { const isSalary = expense.cashierMonthlySalary === true; const isAdvance = expense.cashierSalaryAdvance === true; const details = isSalary ? `راتب ${escapeHtml(expense.cashierName || "كاشير")} · ${expense.month} · السلف ${money(expense.advances)} · خصم العجز ${money(expense.shortageDeductions)}` : `${escapeHtml(expense.description || "بلا وصف")} · ${expense.date}${isAdvance ? ` · سلفة ${escapeHtml(expense.cashierName || "كاشير")}` : ""}${periodType === "monthly" ? " · يوزع على أيام الشهر" : ""}`; return `<article class="entity-row"><div class="entity-row__icon entity-row__icon--expense">${icon(periodType === "monthly" ? "calendar" : "wallet", 20)}</div><div class="entity-row__main"><strong>${escapeHtml(expense.category)}</strong><small>${details}</small></div><strong class="entity-row__amount">${money(expense.amount)}</strong><div class="entity-row__actions">${isSalary ? `<small class="status status--available">راتب شهري</small>` : isAdvance ? `<button class="icon-button" aria-label="حذف سلفة الكاشير" data-action="delete-expense" data-id="${expense.id}">${icon("trash", 18)}</button>` : `<button class="icon-button" aria-label="تعديل المصروف" data-action="edit-expense" data-id="${expense.id}">${icon("edit", 18)}</button><button class="icon-button icon-button--danger" aria-label="حذف المصروف" data-action="delete-expense" data-id="${expense.id}">${icon("trash", 18)}</button>`}</div></article>`; }).join("") : `<div class="inline-empty">لا توجد ${periodType === "monthly" ? "مصروفات شهرية أو رواتب كاشيرات" : "مصروفات يومية"} ضمن الفترة.</div>`;
  ...
  <section class="inventory-summary"><div><span>قيمة المصروفات والرواتب ضمن الفترة</span><strong>${money(total)}</strong></div><div><span>مصروفات يومية</span><strong>${amount(dailyExpenses.length)} عملية</strong></div><div><span>مصروفات شهرية ورواتب</span><strong>${amount(monthlyExpenses.length)} عملية · ${money(salaryTotal)}</strong></div></section>
  <section class="expense-period-grid"><section class="panel entity-list"><div class="panel__head"><div><span class="eyebrow">تشغيل يومي</span><h2>مصروفات يومية</h2></div><small>مثل الأكل والشرب والمواصلات والسلف النقدية</small></div>${expenseRows(dailyExpenses, "daily")}</section><section class="panel entity-list"><div class="panel__head"><div><span class="eyebrow">التزام شهري</span><h2>مصروفات شهرية ورواتب الكاشيرات</h2></div><small>مثل الإيجار والكهرباء والماء ورواتب الكاشيرات</small></div>${expenseRows(monthlyExpenses, "monthly")}</section></section><section class="panel expense-page__bottom-action"><div><span class="eyebrow">سلفة الكاشير</span><strong>تسجيل سلفة جديدة</strong><small>تُخصم من الصندوق والراتب المتبقي</small></div><button class="button button--secondary" data-action="new-cashier-salary-advance">${icon("wallet", 17)}<span>سلفة كاشير</span></button></section>`;
}
