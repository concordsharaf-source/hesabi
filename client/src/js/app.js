...
import { mergeRemoteChanges } from "./sync-domain.js";
...
function expiryStatus(product, today = dateKey()) {
  ...
  const days = Math.ceil((new Date(`${expiryDate}T00:00:00`).getTime() - new Date(`${today}T00:00:00`).getTime()) / 86400000);
  if (days > 90) return null;
  return { expiryDate, days, tone: days <= 30 ? "danger" : "warning", label: days < 0 ? "منتهي" : days <= 30 ? "ينتهي خلال شهر" : "ينتهي خلال 3 أشهر" };
}
...
function inventoryMarkup() {
  ...
  const products = [...state.products].sort((a, b) => priority(a) - priority(b) || a.name.localeCompare(b.name, "ar"));
...
function openAssistantEntryDialog() {
  ...
  overlay.querySelectorAll("[data-dialog-close]").forEach((button) => button.addEventListener("click", closeDialog));
  overlay.querySelector("#assistant-entry-form")?.addEventListener("submit", async (event) => { event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget)); try { await requestAssistantDevice(values); closeDialog(); showToast("تم إرسال الطلب. انتظر موافقة الأدمن ورمز الدخول."); } catch (error) { showToast(error.message || "تعذر إرسال طلب الربط.", "error"); } });
}
...
function openCloudAuthDialog() {
  ...
  overlay.querySelector("#cloud-auth-form").addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (isAdmin(state.currentUser)) { const storeId = state.settings?.cloudStoreId || `store_${randomId()}`; if (!state.settings?.cloudStoreId) { await db.saveSettings({ cloudStoreId: storeId }); state.settings = await db.getSettings(); } state.cloud.identity = await createStoreWorkspace({ storeId, storeName: state.settings?.storeName || "حسابي", ownerAccount: state.currentUser }); try { await watchAssistantRequests(storeId, (requests) => { state.cloud.pairRequests = requests; if (state.view === "data-management") render(); }); } catch (error) { console.warn("[Hesabi pairing requests unavailable]", error); } }
...
function openPairingInviteDialog() {
  ...
  overlay.querySelectorAll("[data-dialog-close]").forEach((button) => button.addEventListener("click", closeDialog));
  overlay.querySelector("#pairing-invite-form")?.addEventListener("submit", async (event) => { event.preventDefault(); const account = state.accounts.find((item) => item.id === new FormData(event.currentTarget).get("accountId")); if (!account) return; try { const storeId = state.settings?.cloudStoreId || `store_${randomId()}`; if (!state.settings?.cloudStoreId) { await db.saveSettings({ cloudStoreId: storeId }); state.settings = await db.getSettings(); } const identity = await createStoreWorkspace({ storeId, storeName: state.settings?.storeName || "حسابي", ownerAccount: state.currentUser }); state.cloud.identity = identity; await seedWorkspaceBackup(await db.exportBackup()); const pairing = await createPairingInvite({ storeId, accountId: account.id, accountName: account.name, role: account.role }); state.cloud.pairing = pairing; closeDialog(); render(); window.prompt("أرسل هذا الرمز للمستخدم مرة واحدة فقط:", pairing.token); } catch (error) { showToast(error.message || "تعذر إنشاء رمز الاقتران.", "error"); } });
}
...
function openPairingRedeemDialog() {
  ...
  overlay.querySelectorAll("[data-dialog-close]").forEach((button) => button.addEventListener("click", closeDialog));
  overlay.querySelector("#pairing-redeem-form")?.addEventListener("submit", async (event) => { event.preventDefault(); try { state.cloud.identity = await redeemPairingInvite(new FormData(event.currentTarget).get("token")); if (state.cloud.identity.bootstrapChanges?.length) { const current = await db.exportBackup(); await db.restoreBackup(mergeRemoteChanges(current, state.cloud.identity.bootstrapChanges)); state.settings = await db.getSettings(); state.accounts = await db.listAccounts(); await refresh(); } closeDialog(); showToast(`تم ربط الجهاز بحساب ${state.cloud.identity.accountName}.`); render(); } catch (error) { showToast(error.message || "تعذر ربط الجهاز.", "error"); } });
}
...
async function downloadReportPdf() { try { const file = await createReportPdfFile({ rows: reportExportRows(), storeName: state.settings?.storeName || "حسابي", logoDataUrl: storeLogoDataUrl(), from: state.reportFrom || "بداية السجل", to: state.reportTo || "اليوم", filename: `hesabi-report-${dateKey()}.pdf` }); downloadGeneratedFile(file); showToast("تم تصدير تقرير PDF"); } catch (error) { showToast(error.message || "تعذر إنشاء تقرير PDF.", "error"); } }
...
async function shareInvoice(invoice) {
  try {
    ...
    const result = await shareOrDownloadInvoicePdf({ invoice: invoiceWithCustomer, customer, storeName: state.settings?.storeName || "حسابي", logoDataUrl: storeLogoDataUrl(), formatMoney: money, formatAmount: amount, formatDateTime: dateTime, paymentLabel: paymentChannelLabel(invoiceWithCustomer), filename: `${invoice.invoiceNumber}.pdf`, title: `فاتورة ${invoice.invoiceNumber}` });
...
async function openCustomerAccountDialog(customerId) {
  ...
  overlay.querySelector("#share-customer-account").addEventListener("click", async () => { try { const result = await shareOrDownloadCustomerAccountPdf({ account: printAccount, storeName: state.settings?.storeName || "حسابي", logoDataUrl: storeLogoDataUrl(), formatMoney: money, formatDateTime: dateTime, filename: `كشف-حساب-${account.customer.name}.pdf`, title: `كشف حساب ${account.customer.name}` }); showToast(result === "shared" ? "تمت مشاركة كشف الحساب PDF" : "تم تنزيل كشف الحساب PDF للمشاركة"); } catch (error) { if (error?.name !== "AbortError") showToast("تعذر إنشاء PDF لكشف الحساب.", "error"); } });
}
...
async function openPurchaseDialog(purchaseIdOrDraft = null) {
  ...
  const makePurchaseLine = (product, values = {}) => {
    ...
    return { productId: product.id, productName: product.name, unit: product.unit || businessProfile().defaultUnit, packageUnit, salePrice: toNumber(salePrice), batchNumber: values.batchNumber || "", expiryDate: values.expiryDate || "", ...values, ...calculatePackagePurchase({ packageQuantity, unitsPerPackage, packageCost }) };
  };
...
export async function bootApp(target) {
  ...
  try { await db.open(); state.settings = await db.getSettings(); state.accounts = await db.listAccounts(); state.currentUser = state.settings?.setupCompleted ? await db.getPersistentSession() : null; try { state.cloud.user = await getCloudBackupUser(); } catch { state.cloud.user = null; } try { state.cloud.identity = await getCloudDeviceIdentity(); } catch { state.cloud.identity = null; } if (state.cloud.identity?.role === "admin" && state.settings?.cloudStoreId) { try { await watchAssistantRequests(state.settings.cloudStoreId, (requests) => { state.cloud.pairRequests = requests; if (state.view === "data-management") render(); }); } catch (error) { console.warn("[Hesabi pairing requests unavailable]", error); } } try { await installSyncCoordinator(db, { onStatus: (status) => { state.cloud.syncStatus = status; }, onRemoteApplied: () => { void refresh().then(render); } }); } catch (error) { state.cloud.syncStatus = "offline"; console.warn("[Hesabi sync unavailable]", error); } applyTheme(); if (state.settings?.setupCompleted) await refresh(); render(); if (state.currentUser?.role === "cashier" && !state.activeCashierShift) requestAnimationFrame(openCashierShiftStartDialog); installExitGuard(); } catch (error) { console.error("[Hesabi boot error]", error); root.innerHTML = `<main class="fatal-state"><img src="${markImage}" alt=""/><h1>تعذر فتح التخزين المحلي</h1><p>لم تُحذف بياناتك المحلية. أعد المحاولة أولًا، واستعد النسخة الاحتياطية فقط عند الحاجة.</p><button class="button button--primary" onclick="location.reload()">إعادة المحاولة</button></main>`; }
}
