...
async function refresh() {
  [state.products, state.productSuppliers, state.sales, state.suppliers, state.supplierPayments, state.customers, state.customerPayments, state.purchases, state.expenses, state.stockMovements, state.cashMovements, state.cashbox, state.dashboard, state.cashierShifts, state.cashierSalarySummaries, state.cashierShiftStatistics, state.vault] = await Promise.all([db.listProducts(), db.listProductSupplierLinks(), db.listSales(), db.listSuppliers(), db.listSupplierPayments(), db.listCustomers(), db.listCustomerPayments(), db.listPurchases(), db.listExpenses(), db.listStockMovements(), db.listCashMovements({ from: state.cashFrom, to: state.cashTo }), db.getCashbox({ from: state.cashFrom, to: state.cashTo }), db.getDashboard(), db.listCashierShifts({ date: "" }), db.listCashierSalarySummaries(), db.listCashierShiftStatistics({ from: state.cashFrom, to: state.cashTo }), db.getVault({ from: state.cashFrom, to: state.cashTo })]);
