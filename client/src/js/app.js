...
function openReportPreview(type = "summary") { const html = reportPreviewHtml(type); const previewDocument = new DOMParser().parseFromString(html, "text/html"); const previewBody = previewDocument.body; const previewTable = previewBody?.querySelector("table"); if (previewTable) { const tableScroller = previewDocument.createElement("div"); tableScroller.className = "report-preview-table-scroll"; previewTable.replaceWith(tableScroller); tableScroller.appendChild(previewTable); } const previewContent = previewBody?.innerHTML || ""; const previewStyles = [...(previewDocument.head?.querySelectorAll("style") || [])].map((style) => style.outerHTML).join(""); const previewWide = (financialReportRows(type)[0] || []).length > 6; const previewWideClass = previewWide ? " report-preview--wide" : ""; const previewHint = previewWide ? '<p class="report-preview__hint">اسحب أفقيًا لعرض جميع الأعمدة.</p>' : ""; const overlay = openDialog(`<div class="dialog__head"><div><span class="eyebrow">معاينة التقرير</span><h2>${escapeHtml(reportTitle(type))}</h2><p class="dialog__subtext">راجع البيانات قبل المشاركة أو الطباعة.</p></div><button class="icon-button" data-dialog-close aria-label="إغلاق">${icon("close", 20)}</button></div><div class="report-preview-scroll"><label class="report-wrap-toggle"><input type="checkbox" data-preview-wrap><span>التفاف النص داخل الخانات</span></label><section class="report-preview report-preview--no-wrap${previewWideClass}" aria-label="معاينة التقرير">${previewStyles}${previewHint}${previewContent}</section><div class="dialog__actions report-preview-actions"><button class="button button--primary" data-preview-share>مشاركة PDF</button><button class="button button--secondary" data-preview-print>طباعة</button><button class="button button--secondary" data-preview-download>تنزيل PDF</button><button class="button button--secondary" data-preview-excel>تنزيل Excel</button><button class="button button--secondary" data-dialog-close>إغلاق</button></div></div>`); overlay.classList.add("dialog-backdrop--report-preview"); overlay.querySelectorAll("[data-dialog-close]").forEach((button) => button.addEventListener("click", closeDialog)); overlay.querySelector("[data-preview-wrap]")?.addEventListener("change", (event) => { overlay.querySelector(".report-preview")?.classList.toggle("report-preview--no-wrap", !event.currentTarget.checked); }); overlay.querySelector("[data-preview-share]").addEventListener("click", async () => { try { const result = await shareReportPdf(type); showToast(result === "shared" ? "تمت مشاركة التقرير PDF" : "تم تنزيل التقرير PDF"); } catch (error) { if (error?.name !== "AbortError") showToast(error.message || "تعذر مشاركة التقرير.", "error"); } }); overlay.querySelector("[data-preview-download]").addEventListener("click", () => void downloadReportPdf(type)); overlay.querySelector("[data-preview-excel]").addEventListener("click", () => downloadReportXlsx(type)); overlay.querySelector("[data-preview-print]").addEventListener("click", () => { printHtmlDocument({ html, target: `hesabi-${type}-report` }); showToast("تم إرسال التقرير للطباعة"); }); }
