...
async function openPurchaseDialog(purchaseIdOrDraft = null) {
  ...
  const renderLines = () => {
    ...
    linesHost.querySelectorAll("[data-purchase-sale-price]").forEach((input) => input.addEventListener("input", (event) => { const index = Number(event.currentTarget.dataset.purchaseSalePrice); lines[index].salePrice = Math.max(0, toNumber(event.currentTarget.value)); const current = linesHost.querySelector(`.purchase-sale-price-field__current`); if (current) current.textContent = money(lines[index].salePrice); }));
