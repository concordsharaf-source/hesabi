...
function openReportPreview(type = "summary") {
  ...
  const previewHeader = `<header><img src="${escapeHtml(storeLogoUrl())}" alt="شعار المتجر"><div><h1>${escapeHtml(storeDisplayName())}</h1><h2>${escapeHtml(reportTitle(type))}</h2><p>من ${escapeHtml(state.reportFrom || "بداية السجل")} إلى ${escapeHtml(state.reportTo || "اليوم")}</p></div></header>`;
