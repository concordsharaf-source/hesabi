...
/* رسالة SMS بالرقم كما هو مسجل تمامًا: أرقام فقط بلا + — نفس مبدأ واتساب المورد. */
const smsExactHref = (phone, text = "") => {
  const digits = String(phone || "").trim().replace(/\D/g, "");
  if (!digits) return "";
  return `sms:${digits}${text ? `?body=${encodeURIComponent(text)}` : ""}`;
};
function sendSmsExact(phone, text) {
  const href = smsExactHref(phone, text);
...
/* زر البيع المباشر الأصلي: يظهر في كل الصفحات عدا المبيعات (هناك يحل محله زر الباركود) */
function directSaleFabMarkup() { return `<button class="direct-sale-fab" data-action="open-sales-scanner" data-mode="sale" aria-label="فتح المبيعات ومسح الباركود" title="بيع ومسح باركود">${icon("cart", 22)}<span>بيع</span></button>`; }
...
/* زر طلب التوريد بجانب المنتج الناقص أو النافد: يفتح نافذة موجهة لمورده المرتبط إن وُجد. */
function supplyRequestButton(product) {
  if (toNumber(product.quantity) > toNumber(product.minimumStock)) return "";
  return `<button class="icon-button icon-button--restock" data-action="product-supply-request" data-id="${product.id}" aria-label="إرسال طلب توريد ${escapeHtml(product.name)}" title="إرسال طلب توريد للمورد">${icon("truck", 18)}</button>`;
}
function openProductSupplyRequestDialog(productId) {
  ...
  const validate = () => {
    ...
    if (!supplier) { showToast("اختر المورد أولًا.", "error"); return null; }
  ...
  overlay.querySelector("#psr-send-whatsapp").addEventListener("click", () => { const order = validate(); if (order) sendWhatsAppExact(order.supplier.phone, requestText(order)); });
  overlay.querySelector("#psr-send-sms").addEventListener("click", () => { const order = validate(); if (order) sendSmsExact(order.supplier.phone, requestText(order)); });
}
...
function openReorderDialog() {
  ...
  overlay.querySelectorAll("[data-reorder-send-sms]").forEach((button) => button.addEventListener("click", () => {
    const key = button.dataset.reorderSendSms; const group = groups.get(key); const lines = requestedLines(key);
    if (!lines.length) { showToast("حدّد كمية لصنف واحد على الأقل.", "error"); return; }
    sendSmsExact(group.supplier?.phone, orderText(group, lines));
  }));
  overlay.querySelectorAll("[data-reorder-send-image]").forEach((button) => button.addEventListener("click", async () => {
    const key = button.dataset.reorderSendImage; const group = groups.get(key); const lines = requestedLines(key);
    if (!lines.length) { showToast("حدّد كمية لصنف واحد على الأقل.", "error"); return; }
    await shareDocumentImage({ html: orderHtml(group, lines), filename: `طلب-توريد-${dateKey()}.png`, title: `طلب توريد — ${group.supplier?.name || ""}`, button });
  }));
...
function cartLine(line) {
  ...
  const quantityInput = soldAsPackage ? `<input data-cart-carton-count="${line.productId}" type="number" lang="en" inputmode="numeric" min="1" ${state.settings?.allowNegativeSales ? "" : `max="${Math.floor(toNumber(product?.quantity) / unitsPerPackage)}"`} step="1" value="${cartonCount}" aria-label="عدد الكراتين" />` : `<input data-cart-quantity="${line.productId}" type="number" lang="en" inputmode="decimal" min="1" ${state.settings?.allowNegativeSales ? "" : `max="${toNumber(product?.quantity)}"`} step="1" value="${line.quantity}" aria-label="عدد الحبات" />`;
...
function suppliersMarkup() {
  ...
  <section class="inventory-summary inventory-summary--purchases"><div><span>مستحقات الموردين</span><strong>${money(totalDue)}</strong></div><button class="po-order-tile" type="button" data-action="new-purchase-order" aria-label="عمل طلب شراء وإرساله للمورد"><span>${icon("truck", 15)} مراسلة المورد</span><strong>طلب شراء</strong></button><button class="po-order-tile" type="button" data-action="new-supplier" aria-label="إضافة مورد جديد"><span>${icon("plus", 15)} سجل موردًا جديدًا</span><strong>إضافة مورد</strong></button></section>
...
function purchasesMarkup() {
  return `${topbarMarkup("المشتريات", "أنشئ فاتورة شراء لزيادة المخزون وتثبيت تكلفة المنتجات، مع إمكانية ربط المورد عند توفره.")}
  <section class="inventory-summary inventory-summary--purchases"><div><span>إجمالي المشتريات</span><strong>${money(state.analytics?.purchases.total || 0)}</strong></div><button class="po-order-tile" type="button" data-action="new-purchase" aria-label="إضافة فاتورة شراء جديدة"><span>${icon("plus", 15)} ${amount(state.purchases.length)} فاتورة مسجلة</span><strong>إضافة فاتورة</strong></button><button class="po-order-tile" type="button" data-action="new-purchase-order" aria-label="عمل طلب شراء وإرساله للمورد"><span>${icon("truck", 15)} مراسلة المورد</span><strong>طلب شراء</strong></button></section>
...
function openPurchaseOrderDialog() {
  ...
  const itemRow = () => `<div class="po-line"><div class="po-line__name-wrap"><input class="po-line__name" dir="rtl" placeholder="اسم الصنف المطلوب" aria-label="اسم الصنف" autocomplete="off" /><div class="po-suggest" hidden></div></div><input class="po-line__qty" type="number" lang="en" inputmode="decimal" min="0" step="1" placeholder="الكمية" aria-label="الكمية" /><select class="po-line__unit" aria-label="بالحبة أو بنوع الكمية">${PACKAGE_UNITS.map((unit) => `<option value="${unit}">${unit}</option>`).join("")}</select><button class="icon-button icon-button--danger po-line__remove" type="button" aria-label="حذف السطر">${icon("close", 16)}</button></div>`;
  ...
  overlay.querySelector("#po-send-sms").addEventListener("click", () => {
    const order = validate(); if (!order) return;
    if (!order.supplier.phone) { showToast("لا يوجد رقم مسجل لهذا المورد — أضف رقمه أولًا.", "error"); return; }
    sendSmsExact(order.supplier.phone, orderText(order));
  });
  ...
  overlay.querySelector("#po-share-image").addEventListener("click", async (event) => {
    const order = validate(); if (!order) return;
    await shareDocumentImage({ html: orderHtml(order), filename: `طلب-شراء-${dateKey()}.png`, title: `طلب شراء — ${order.supplier.name}`, button: event.currentTarget });
  });
}
...
function cloudBackupMarkup() {
  ...
  if (!cloud.user) {
    return `<section class="panel report-card cloud-backup-card"><span class="eyebrow">نسخ سحابي مجاني</span><h2>نسخة آمنة بين الأجهزة</h2><p>اربط حسابك مرة واحدة لتحفظ نسخة من بياناتك في السحابة، وتستعيدها متى احتجت على هذا الجهاز أو أي جهاز آخر.</p><div class="cloud-backup-card__note"><strong>بياناتك بأمان</strong><span>يعمل التطبيق دون إنترنت وتبقى بياناتك على جهازك، والنسخة السحابية احتياط إضافي ترفعه وتستعيده وقت ما تشاء.</span></div><div class="dialog__actions"><button class="button button--primary" data-action="open-cloud-auth">ربط النسخ السحابية</button></div></section>`;
  }
  ...
  return `<section class="panel report-card cloud-backup-card"><div class="panel__head"><div><span class="eyebrow">نسخ سحابي مجاني</span><h2>نسخ ${escapeHtml(cloud.user.email || "السحابية")}</h2></div><button class="text-button" data-action="cloud-signout" ${busy ? "disabled" : ""}>فصل الحساب</button></div><p>تُحفظ نسخك الاحتياطية في السحابة كما هي، وتستعيدها على هذا الجهاز أو أي جهاز آخر متى شئت. تتحدث القائمة تلقائيًا بعد كل رفع أو حذف.</p><div class="dialog__actions"><button class="button button--primary" data-action="cloud-upload-backup" ${busy ? "disabled" : ""}>${busy === "upload" ? "جارٍ رفع النسخة…" : "إنشاء نسخة سحابية الآن"}</button></div>${cloud.error ? `<p class="cloud-backup-error">${escapeHtml(cloud.error)}</p>` : ""}<div class="cloud-backup-list">${cloud.loading ? `<div class="inline-empty">جارٍ تحميل النسخ السحابية…</div>` : rows || `<div class="inline-empty">لا توجد نسخة سحابية بعد. أنشئ أول نسخة بعد مراجعة بيانات جهازك.</div>`}</div></section>`;
}
...
function renderApplication() {
  ...
  root.innerHTML = `<div class="app-shell">${navMarkup()}<main class="workspace">${body}</main>${salesScannerFabMarkup()}${directSaleFabMarkup()}</div>`;
...
function render() {
  ...
  requestAnimationFrame(() => {
    ...
    try { document.documentElement.classList.toggle("is-sales-page", state.view === "sales"); } catch { /* تجميلي */ }
  });
}
...
async function handleActionUnsafe(event) {
  ...
  if (action === "product-supply-request") { openProductSupplyRequestDialog(id); return; }
...
function openAccountDialog(account = null) {
  ...
  roleSelect.closest("label").insertAdjacentHTML("afterend", `<label id="account-salary-field">الراتب الشهري<input name="monthlySalary" type="number" lang="en" min="0" step="0.01" inputmode="decimal" value="${escapeHtml(account?.monthlySalary ?? "")}" placeholder="0" /><small class="field-hint">يستخدم لحساب السلف والراتب المتبقي للموظف.</small></label>`);
...
function productFormMarkup(product = null, presetBarcode = "") {
  ...
  const input = (name, label, type = "text", value = "", attrs = "") => `<label>${label}<input name="${name}" type="${type}" ${type === "number" ? 'lang="en" ' : ""}value="${escapeHtml(value)}" ${attrs} /></label>`;
  ...
  /* قسم نوع الكمية: نفس منطق فاتورة الشراء — نوع الكمية، عدد الحبات فيها، وسعرها. */
  const packageSection = `
    <fieldset class="form-full product-package-fieldset">
      <legend><span class="eyebrow">التعبئة والتسعير</span></legend>
      <label class="checkbox-field product-package-toggle"><input type="checkbox" id="product-package-mode" ${unitsPerPackage > 1 ? "checked" : ""} /><span><strong>الإدخال بنوع الكمية (كراتين)</strong><small>احسب سعر ${escapeHtml(unit)} تلقائيًا من سعر نوع الكمية</small></span></label>
      <div class="product-package-grid" id="product-package-grid" ${unitsPerPackage > 1 ? "" : "hidden"}>
        <label>نوع الكمية<select name="packageUnit">${profileOptions("packageUnits", packageUnit).map((item) => `<option value="${item}" ${item === packageUnit ? "selected" : ""}>${item}</option>`).join("")}</select></label>
        <label><span data-package-label="units">${labels.units}</span><input name="unitsPerPackage" type="number" lang="en" min="1" step="1" value="${unitsPerPackage}" /></label>
        <label><span data-package-label="cost">${labels.cost}</span><input name="packageCost" type="number" lang="en" min="0" step="0.01" value="${packageCost || ""}" /></label>
        ${!isEdit ? `<label><span data-package-label="quantity">${labels.quantity}</span><input name="packageQuantity" type="number" lang="en" min="0" step="1" value="" placeholder="0" /></label>` : ""}
      </div>
...
/* يربط حقول نوع الكمية في نموذج المنتج: يحدّث المسميات ويحسب سعر الوحدة والكمية تلقائيًا. */
function bindProductPackageFields(overlay, form, { isEdit = false } = {}) {
  ...
  const recalc = () => {
    ...
    // سعر شراء الوحدة يُشتق من سعر نوع الكمية، والكمية تُشتق من عدد الكميات.
...
function openCloudAuthDialog(initialMode = "signin") {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      showError(permissionIssue ? "تم إنشاء الحساب لكن تعذر تجهيز مساحة المتجر في السحابة. انتظر لحظات ثم أعد المحاولة من الزر نفسه." : error.message || "تعذر ربط حساب النسخ السحابية.");
    }
  });
}
...
function openReportPreview(type = "summary") {
  ...
  overlay.querySelector("[data-preview-share-image]").addEventListener("click", (event) => shareDocumentImage({ html, filename: `hesabi-${type}-report-${dateKey()}.png`, title: reportTitle(type), button: event.currentTarget }));
      ...
      <div class="cash-calc-body">
        <div class="cash-calc-inputs">
          <label class="cash-tendered-label">
            ...
            <input id="cash-tendered" type="number" lang="en" inputmode="decimal" min="0" step="0.01" value="${initial.total}" placeholder="أدخل الواصل" />
          </label>
        </div>
...
async function openInvoiceDialog(saleId) {
  ...
  overlay.querySelector(".dialog__actions").insertAdjacentHTML("afterbegin", `<button id="share-invoice" class="button button--secondary" type="button">مشاركة PDF</button><button id="share-invoice-image" class="button button--secondary" type="button">مشاركة صورة</button><button id="whatsapp-invoice" class="button button--secondary" type="button">${icon("whatsapp", 17)} واتساب</button><button id="thermal-print-invoice" class="button button--secondary" type="button">طباعة حرارية</button>`);
  overlay.querySelector("#share-invoice").addEventListener("click", () => shareInvoice(invoice));
  overlay.querySelector("#share-invoice-image").addEventListener("click", async (event) => { const invoiceWithCustomer = { ...invoice, customerName: invoice.customerName || state.customers.find((item) => item.id === invoice.customerId)?.name || "" }; await shareDocumentImage({ html: await thermalInvoiceHtml(invoiceWithCustomer), filename: `${invoice.invoiceNumber}.png`, title: `فاتورة ${invoice.invoiceNumber}`, page: "thermal", button: event.currentTarget }); });
...
async function openCustomerAccountDialog(customerId) {
  ...
  overlay.querySelector("#share-customer-account-image").addEventListener("click", (event) => shareDocumentImage({ html: accountHtml(), filename: `كشف-حساب-${account.customer.name}.png`, title: `كشف حساب ${account.customer.name}`, button: event.currentTarget }));
...
async function openPurchaseDialog(purchaseIdOrDraft = null) {
  ...
  const packageLabels = (packageUnit) => ({ "حبة": { quantity: "عدد الحبات", units: "حبة/حبة", cost: "سعر الحبة" }, "علبة": { quantity: "عدد العلب", units: "حبة/علبة", cost: "سعر العلبة" }, "كرتون": { quantity: "عدد الكراتين", units: "حبة/كرتون", cost: "سعر الكرتون" }, "كيس": { quantity: "عدد الأكياس", units: "حبة/كيس", cost: "سعر الكيس" }, "حزمة": { quantity: "عدد الحزم", units: "حبة/حزمة", cost: "سعر الحزمة" }, "ربطة": { quantity: "عدد الربطات", units: "حبة/ربطة", cost: "سعر الربطة" }, "صندوق": { quantity: "عدد الصناديق", units: "حبة/صندوق", cost: "سعر الصندوق" } }[packageUnit] || { quantity: "عدد الكميات", units: "حبات/نوع الكمية", cost: "سعر نوع الكمية" });
  const renderLines = () => {
    linesHost.innerHTML = lines.length ? lines.map((line, index) => { const labels = packageFieldLabels(line.packageUnit, line.unit); return `<article class="purchase-line purchase-line--pack"><div><strong>${escapeHtml(line.productName)}</strong><small>سيضاف ${amount(line.quantity)} ${escapeHtml(line.unit)} إلى المخزون</small></div><label>نوع الكمية<select data-purchase-package-unit="${index}">${PACKAGE_UNITS.map((unit) => `<option value="${unit}" ${line.packageUnit === unit ? "selected" : ""}>${unit}</option>`).join("")}</select></label><label>${labels.quantity}<input data-purchase-package-quantity="${index}" type="number" lang="en" inputmode="decimal" min="0.001" step="0.001" value="${line.packageQuantity || ""}" /></label><label>${labels.units}<input data-purchase-units-per-package="${index}" type="number" lang="en" inputmode="numeric" min="1" step="1" value="${line.unitsPerPackage || ""}" /></label><label>${labels.cost}<input data-purchase-package-cost="${index}" type="number" lang="en" inputmode="decimal" min="0" step="0.01" value="${line.packageCost || ""}" /></label><label>${labels.summary}<output>${money(line.unitCost)}</output></label><strong class="purchase-line__total">${money(line.total)}</strong><button data-remove-purchase-line="${index}" class="icon-button icon-button--danger" type="button" aria-label="حذف">${icon("close", 17)}</button></article>`; }).join("") : `<div class="inline-empty">أضف منتجًا واحدًا على الأقل.</div>`;
...
function openPurchaseProductDialog(draft) {
  ...
  overlay.querySelectorAll("[data-dialog-close]").forEach((button) => button.addEventListener("click", closeDialog)); quickForm.addEventListener("submit", async (event) => { event.preventDefault(); try { const values = Object.fromEntries(new FormData(event.currentTarget)); if (values.category === "__custom__") values.category = String(values.customCategory || "").trim() || "أخرى"; if (await checkBarcode()) return; const math = calculatePackagePurchase(values); if (math.packageQuantity <= 0 || math.unitsPerPackage <= 0) throw new Error("أدخل عدد الكميات ووحدات المخزون بصورة صحيحة."); const product = await db.createProduct({ ...values, quantity: 0, purchasePrice: math.unitCost, purchasePackageUnit: values.packageUnit, lastPackageCost: math.packageCost }); await refresh(); closeDialog(); draft.lines.push({ productId: product.id, productName: product.name, unit: product.unit, packageUnit: values.packageUnit, salePrice: values.salePrice, batchNumber: values.batchNumber || "", productionDate: values.productionDate || "", expiryDate: values.expiryDate || "", ...math }); openPurchaseDialog(draft); } catch (error) { showToast(error.message, "error"); } });
}
...
/* مشاركة أي فاتورة أو تقرير كصورة بنمط الطابعة (أبيض/أسود) — تُستخدم في كل الأقسام. */
async function shareDocumentImage({ html, filename, title, page = "a4", button = null }) {
  if (button) button.disabled = true;
  try {
    const result = await shareOrDownloadImage({ html, filename, title, page });
    if (result === "downloaded") showToast("نُزّلت الصورة — شاركها أو اطبعها.");
  } catch (error) { if (error?.name !== "AbortError") showToast(error.message || "تعذر إنشاء الصورة.", "error"); }
...
function openPurchaseDetail(purchase) {
  ...
  overlay.querySelector("#share-purchase-image").addEventListener("click", async (event) => { const supplier = state.suppliers.find((item) => item.id === purchase.supplierId) || null; const normalizedPurchase = normalizePurchaseSalePrices(purchase); const purchaseForImage = { ...normalizedPurchase, supplierPhone: supplier?.phone || "", supplierAddress: supplier?.address || "" }; const html = renderPurchaseInvoiceHtml({ purchase: purchaseForImage, supplier, storeName: storeDisplayName(), storeInfo: state.settings, logoDataUrl: storeLogoDataUrl() || storeLogoUrl(), formatMoney: money, formatAmount: amount, formatDateTime: dateTime, escapeHtml }); await shareDocumentImage({ html, filename: `${purchase.invoiceNumber}.png`, title: `فاتورة شراء ${purchase.invoiceNumber}`, button: event.currentTarget }); });
