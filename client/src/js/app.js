...
import { createReportPdfFile, printHtmlDocument, shareOrDownloadCustomerAccountPdf, shareOrDownloadReportPdf, shareOrDownloadInvoicePdf, shareOrDownloadPdf, shareOrDownloadPurchaseInvoicePdf } from "./pdf-export.js";
...
const reportTitle = (type = "summary") => ({ cash: "تقرير تحليلي شامل لحركة الصندوق", income: "قائمة الدخل", balance: "المركز المالي", trial: "ميزان المراجعة", customers: "تقرير ديون العملاء", suppliers: "تقرير مستحقات الموردين", expenses: "تقرير المصروفات حسب النوع", inventory: "تقرير المخزون والتكلفة", summary: "التقرير التشغيلي العام" }[type] || "التقرير المالي");
function reportPreviewHtml(type = "summary") { const rows = financialReportRows(type); const title = reportTitle(type); const cell = (value, header = false) => `<${header ? "th" : "td"}>${escapeHtml(value ?? "")}</${header ? "th" : "td"}>`; return `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><style>@page{size:A4;margin:12mm}*{box-sizing:border-box}body{font-family:Tahoma,Arial,sans-serif;color:#172e27;background:#fff;direction:rtl;margin:0;font-size:16px}header{display:flex;align-items:center;gap:18px;border:2px solid #174c3f;border-radius:10px;padding:16px 20px;margin-bottom:18px;background:#f5faf7}header img{width:74px;height:74px;object-fit:contain;background:#fff;border:1px solid #b7cdbf;border-radius:8px}header div{flex:1;text-align:right}h1{margin:0 0 8px;font-size:25px}h2{margin:0 0 5px;font-size:20px}p{margin:0;color:#52645b;font-size:14px}table{width:100%;border-collapse:collapse;table-layout:fixed;font-size:16px}th,td{padding:11px 10px;border:1.5px solid #52645b;text-align:right;vertical-align:middle;overflow-wrap:anywhere;line-height:1.5}th{background:#dfece4;font-size:17px;font-weight:800}td:not(:first-child),th:not(:first-child){text-align:center}tr:nth-child(even){background:#f8fbf9}</style></head><body><header><img src="${escapeHtml(storeLogoUrl())}" alt="شعار المتجر"><div><h1>${escapeHtml(storeDisplayName())}</h1><h2>${escapeHtml(title)}</h2><p>من ${escapeHtml(state.reportFrom || "بداية السجل")} إلى ${escapeHtml(state.reportTo || "اليوم")}</p></div></header><table><thead><tr>${(rows[0] || []).map((value) => cell(value, true)).join("")}</tr></thead><tbody>${rows.slice(1).map((row) => `<tr>${row.map((value) => cell(value)).join("")}</tr>`).join("")}</tbody></table></body></html>`; }
async function downloadReportPdf(type = "summary") { try { const file = await createReportPdfFile({ rows: financialReportRows(type), storeName: storeDisplayName(), storeInfo: state.settings, logoDataUrl: storeLogoDataUrl() || storeLogoUrl(), from: state.reportFrom || "بداية السجل", to: state.reportTo || "اليوم", filename: `hesabi-${type}-report-${dateKey()}.pdf` }); downloadGeneratedFile(file); showToast("تم تصدير التقرير PDF"); } catch (error) { showToast(error.message || "تعذر إنشاء تقرير PDF.", "error"); } }
async function shareReportPdf(type = "summary") { return shareOrDownloadReportPdf({ rows: financialReportRows(type), storeName: storeDisplayName(), storeInfo: state.settings, logoDataUrl: storeLogoDataUrl() || storeLogoUrl(), from: state.reportFrom || "بداية السجل", to: state.reportTo || "اليوم", filename: `hesabi-${type}-report-${dateKey()}.pdf`, title: reportTitle(type) }); }
function openReportPreview(type = "summary") { const html = reportPreviewHtml(type); const overlay = openDialog(`<div class="dialog__head"><div><span class="eyebrow">معاينة التقرير</span><h2>${escapeHtml(reportTitle(type))}</h2><p class="dialog__subtext">راجع البيانات قبل المشاركة أو الطباعة.</p></div><button class="icon-button" data-dialog-close aria-label="إغلاق">${icon("close", 20)}</button></div><label class="report-wrap-toggle"><input type="checkbox" data-preview-wrap checked><span>التفاف النص داخل الخانات</span></label><section class="report-preview" aria-label="معاينة التقرير">${html.replace(/^<!doctype html><html[^>]*><head>[\s\S]*?<\/head><body>|<\/body><\/html>$/gi, "")}</section><div class="dialog__actions report-preview-actions"><button class="button button--primary" data-preview-share>مشاركة PDF</button><button class="button button--secondary" data-preview-print>طباعة</button><button class="button button--secondary" data-preview-download>تنزيل PDF</button><button class="button button--secondary" data-preview-excel>تنزيل Excel</button><button class="button button--secondary" data-dialog-close>إغلاق</button></div>`); overlay.querySelectorAll("[data-dialog-close]").forEach((button) => button.addEventListener("click", closeDialog)); overlay.querySelector("[data-preview-wrap]")?.addEventListener("change", (event) => { overlay.querySelector(".report-preview")?.classList.toggle("report-preview--no-wrap", !event.currentTarget.checked); }); overlay.querySelector("[data-preview-share]").addEventListener("click", async () => { try { const result = await shareReportPdf(type); showToast(result === "shared" ? "تمت مشاركة التقرير PDF" : "تم تنزيل التقرير PDF"); } catch (error) { if (error?.name !== "AbortError") showToast(error.message || "تعذر مشاركة التقرير.", "error"); } }); overlay.querySelector("[data-preview-download]").addEventListener("click", () => void downloadReportPdf(type)); overlay.querySelector("[data-preview-excel]").addEventListener("click", () => downloadReportXlsx(type)); overlay.querySelector("[data-preview-print]").addEventListener("click", () => { printHtmlDocument({ html, target: `hesabi-${type}-report` }); showToast("تم إرسال التقرير للطباعة"); }); }
