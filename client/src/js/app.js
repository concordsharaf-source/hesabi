...
async function openInvoiceDialog(saleId) {
  ...
  overlay.querySelector(".dialog__actions").insertAdjacentHTML("afterbegin", `<button id="thermal-print-invoice" class="button button--secondary" type="button">طباعة حرارية</button>`);
  overlay.querySelector("#thermal-print-invoice").addEventListener("click", () => printInvoiceThermal(invoice));
}

function printHtmlDocument({ html, target, features }) {
...
async function openCustomerAccountDialog(customerId) {
  ...
  overlay.querySelector("#print-customer-account").addEventListener("click", () => { if (!printHtmlDocument({ html: renderCustomerAccountHtml({ account: printAccount, storeName: storeDisplayName(), formatMoney: money, formatDateTime: dateTime, escapeHtml }), target: "hesabi-customer-account", features: "width=900,height=760" })) showToast("السماح بالنوافذ المنبثقة مطلوب للطباعة.", "error"); });
}
