...
function salesMarkup() {
  ...
  <div class="cart-total"><div class="cart-total__summary"><div><span>الإجمالي المبدئي</span><strong data-cart-subtotal>${money(totals.subtotal - totals.discount)}</strong></div><label class="cart-discount-field">خصم السلة<input data-cart-discount type="text" inputmode="decimal" placeholder="0 أو 20%" value="${escapeHtml(state.cartDiscount || "")}" /><small data-cart-discount-hint>${totals.discount ? `خصم ${money(totals.discount)}` : "مبلغ أو نسبة"}</small></label></div><button class="button button--primary button--wide checkout-launch" data-action="checkout" ${state.cart.length ? "" : "disabled"}>إتمام البيع ${icon("arrow", 18)}</button></div></aside></section>`;
}

function cartLine(line) {
  const product = state.products.find((item) => item.id === line.productId);
  const lineSubtotal = roundMoney(toNumber(line.unitPrice) * toNumber(line.quantity)); const lineDiscount = calculateDiscountAmount(line.discount, lineSubtotal); const lineTotal = roundMoney(lineSubtotal - lineDiscount);
  return `<article class="cart-line"><div class="cart-line__detail"><strong class="arabic-product-name" dir="rtl" lang="ar">${escapeHtml(line.name)}</strong><small>${money(line.unitPrice)} × ${amount(line.quantity)}</small></div><strong data-cart-line-total="${line.productId}">${money(lineTotal)}</strong><div class="quantity-control quantity-control--dark"><button aria-label="إنقاص" data-action="cart-decrement" data-id="${line.productId}">${icon("minus", 15)}</button><input data-cart-quantity="${line.productId}" type="number" inputmode="decimal" min="1" max="${toNumber(product?.quantity)}" step="1" value="${line.quantity}" /><button aria-label="زيادة" data-action="cart-increment" data-id="${line.productId}">${icon("plus", 15)}</button></div><label class="cart-line__discount" title="خصم هذا الصنف">خصم<input data-cart-line-discount="${line.productId}" type="text" inputmode="decimal" placeholder="0 أو %" value="${escapeHtml(line.discount || "")}" /></label><button class="remove-line" aria-label="حذف من السلة" data-action="cart-remove" data-id="${line.productId}">${icon("close", 16)}</button></article>`;
}
...
function bindEvents() {
  ...
  root.querySelectorAll("[data-cart-discount]").forEach((input) => input.addEventListener("input", (event) => {
    state.cartDiscount = event.currentTarget.value;
    const totals = calculateSaleTotals(state.cart, state.cartDiscount);
    const subtotal = root.querySelector("[data-cart-subtotal]"); const hint = root.querySelector("[data-cart-discount-hint]");
    if (subtotal) subtotal.textContent = money(totals.total); if (hint) hint.textContent = totals.discount ? `خصم ${money(totals.discount)}` : "مبلغ أو نسبة";
  }));
  root.querySelectorAll("[data-cart-discount]").forEach((input) => input.addEventListener("change", () => render()));
  root.querySelectorAll("[data-cart-line-discount]").forEach((input) => input.addEventListener("input", (event) => {
    const line = state.cart.find((item) => item.productId === event.currentTarget.dataset.cartLineDiscount); if (!line) return; line.discount = event.currentTarget.value;
    const lineSubtotal = roundMoney(toNumber(line.unitPrice) * toNumber(line.quantity)); const lineTotal = roundMoney(lineSubtotal - calculateDiscountAmount(line.discount, lineSubtotal)); const total = root.querySelector(`[data-cart-line-total="${line.productId}"]`); if (total) total.textContent = money(lineTotal);
    const totals = calculateSaleTotals(state.cart, state.cartDiscount); const subtotal = root.querySelector("[data-cart-subtotal]"); if (subtotal) subtotal.textContent = money(totals.total);
  }));
  root.querySelectorAll("[data-cart-line-discount]").forEach((input) => input.addEventListener("change", () => render()));
}
...
function openCheckoutDialog() {
  const initial = calculateSaleTotals(state.cart, state.cartDiscount);
  ...
  const syncCheckout = () => { state.cartDiscount = form.discount.value; const totals = totalsForForm(); const isCredit = form.paymentType.value === "آجل"; const delivery = Math.max(0, toNumber(form.deliveryFee.value)); const deliveryForCustomer = form.deliveryChargeType.value === "customer" && delivery > 0; const needsCustomer = isCredit || deliveryForCustomer; paidInput.max = totals.total; paidInput.disabled = isCredit; paidInput.value = isCredit ? 0 : totals.total; const remaining = Math.max(0, totals.total - toNumber(paidInput.value)); overlay.querySelector("#checkout-total").textContent = money(totals.total); overlay.querySelector("#remaining-amount").textContent = money(remaining); overlay.querySelector("#payment-status").textContent = isCredit ? "غير مدفوعة — يُسجل التحصيل لاحقًا من حساب العميل" : deliveryForCustomer ? "مدفوعة المنتجات — التوصيل يُضاف لحساب العميل" : "مدفوعة"; creditField.hidden = !needsCustomer; creditSummary.hidden = !isCredit; };
