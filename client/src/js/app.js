...
function expiryMeterMarkup(product) { const progress = expiryProgress({ productionDate: product?.nearestProductionDate, expiryDate: product?.nearestExpiryDate }); if (!progress) return `<div class="expiry-meter expiry-meter--empty" title="لا توجد تواريخ إنتاج وانتهاء مكتملة"><span></span></div>`; const ratio = Math.max(0, Math.min(1, progress.ratio)); const tone = progress.ratio >= 1 ? "danger" : progress.ratio >= 0.85 ? "warning" : "safe"; return `<div class="expiry-meter expiry-meter--${tone}" title="استهلك ${Math.round(Math.min(100, Math.max(0, progress.ratio * 100)))}% من مدة الصلاحية"><span style="width:${Math.round(ratio * 100)}%"></span></div>`; }
...
async function deleteAllProducts() { const count = state.products.length; if (!count) { showToast("لا توجد منتجات لحذفها."); return; } if (!window.confirm(`سيتم حذف ${count} منتجًا من قوائم المنتجات والمخزون. ستبقى الفواتير والسجلات المالية محفوظة. هل تريد المتابعة؟`)) return; try { const deleted = await db.softDeleteAllProducts(); state.cart = []; await refresh(); render(); showToast(`تم حذف ${deleted} منتجًا من القوائم مع الحفاظ على السجلات.`); } catch (error) { showToast(error.message || "تعذر حذف المنتجات.", "error"); } }
