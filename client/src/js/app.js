...
function openAccountDialog(account = null) {
  ...
  const syncSalaryField = () => { const staff = roleSelect.value === "cashier" || roleSelect.value === "employee"; salaryField.hidden = !staff; salaryField.querySelector("input").disabled = !staff; };
...
async function saveSettings(event) { event.preventDefault(); try { const values = Object.fromEntries(new FormData(event.currentTarget)); values.allowNegativeSales = Boolean(event.currentTarget.querySelector("[name=allowNegativeSales]")?.checked); await db.saveSettings(values); state.settings = await db.getSettings(); await refresh(); render(); showToast("تم حفظ إعدادات المتجر"); } catch (error) { showToast(error.message, "error"); } }
...
async function settleStaffSalary(accountId) {
  ...
  if (!account || !["cashier", "employee"].includes(account.role) || !summary) { showToast("لا توجد بيانات راتب لهذا الموظف في الشهر الحالي.", "error"); return; }
  if (summary.salaryDelivered) { showToast("تم تسليم راتب هذا الموظف لهذا الشهر مسبقًا.", "error"); return; }
  const message = `سيتم تسليم ${money(summary.remainingSalary)} للموظف ${account.name} عن شهر ${summary.month} بعد خصم السلف وخصومات العجز، وسيُحتسب الراتب الكامل ${money(summary.monthlySalary)} ضمن رواتب المصروفات. هل تريد المتابعة؟`;
...
function openCashierSalaryAdvanceDialog() {
  const staff = state.accounts.filter((account) => ["cashier", "employee"].includes(account.role) && account.isActive);
  if (!staff.length) { showToast("أضف حساب موظف أو كاشير نشطًا قبل تسجيل السلفة.", "error"); return; }
...
function purchaseInvoiceHtml(purchase) {
  const rows = (purchase.items || []).map((item) => `<tr><td>${escapeHtml(item.productName)}</td><td>${amount(item.quantity)} ${escapeHtml(item.unit || "")}</td><td>${money(item.unitCost)}</td><td>${money(item.total)}</td></tr>`).join("");
  return `<!doctype html><html dir="rtl" lang="ar"><head><meta charset="utf-8"><style>body{font-family:Arial,sans-serif;color:#17251f;padding:24px;direction:rtl}h1{margin:0 0 6px;color:#1f6f52}p{margin:5px 0;color:#53645d}.meta{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:22px 0;padding:14px;background:#f1f6f2;border-radius:10px}table{width:100%;border-collapse:collapse;margin-top:18px}th,td{border-bottom:1px solid #d9e3dd;padding:10px;text-align:right}th{background:#e8f1eb}.total{margin-top:20px;display:flex;justify-content:space-between;font-size:20px;font-weight:700}</style></head><body><h1>${escapeHtml(storeDisplayName())}</h1><h2>فاتورة شراء ${escapeHtml(purchase.invoiceNumber)}</h2><div class="meta"><div><strong>المورد:</strong> ${escapeHtml(purchase.supplierName || "بدون مورد")}</div><div><strong>التاريخ:</strong> ${escapeHtml(dateTime(purchase.date))}</div><div><strong>نوع الدفع:</strong> ${escapeHtml(purchase.paymentType || "نقدي")}</div><div><strong>المتبقي:</strong> ${money(purchase.remainingAmount)}</div></div><table><thead><tr><th>المنتج</th><th>الكمية</th><th>سعر الوحدة</th><th>الإجمالي</th></tr></thead><tbody>${rows}</tbody></table><div class="total"><span>إجمالي الفاتورة</span><strong>${money(purchase.total)}</strong></div>${purchase.notes ? `<p><strong>ملاحظات:</strong> ${escapeHtml(purchase.notes)}</p>` : ""}</body></html>`;
}
