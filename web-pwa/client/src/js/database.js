...
export const db = {
  ...
  async completeSale({ items, discount, paidAmount, paymentMethod, paymentType = "نقدي", customerId = "" }) {
    ...
    const database = await this.open(); const transaction = database.transaction(["settings", "products", "productBatches", "sales", "saleItems", "stockMovements", "meta", "customers", "customerTransactions"], "readwrite"); const settings = await requestAsPromise(transaction.objectStore("settings").get("app")); const pharmacyMode = settings?.businessType === "صيدلية"; const products = transaction.objectStore("products"); const batches = transaction.objectStore("productBatches"); const resolved = []; const today = dateKey();
    for (const line of items) { const product = await requestAsPromise(products.get(line.productId)); if (!product || product.isDeleted) throw new Error("أحد منتجات السلة لم يعد متاحًا."); if (!canSell(product.quantity, line.quantity)) throw new Error(`الكمية المتوفرة غير كافية للمنتج: ${product.name}`); const availableBatches = pharmacyMode ? (await requestAsPromise(batches.index("productId").getAll(product.id))).filter((batch) => toNumber(batch.remainingQuantity) > 0 && batch.expiryDate && batch.expiryDate >= today).sort((a, b) => String(a.expiryDate).localeCompare(String(b.expiryDate))) : []; if (pharmacyMode && availableBatches.reduce((sum, batch) => sum + toNumber(batch.remainingQuantity), 0) < toNumber(line.quantity)) throw new Error(`لا توجد كميات صالحة كافية ضمن تشغيلات المنتج: ${product.name}`); resolved.push({ line, product, availableBatches }); }
    const totals = calculateSaleTotals(resolved.map(({ line, product }) => ({ unitPrice: product.salePrice, quantity: line.quantity })), discount); const isCredit = paymentType === "آجل"; const paid = isCredit ? 0 : Math.max(0, toNumber(paidAmount)); if (paid > totals.total) throw new Error("المبلغ المدفوع لا يمكن أن يتجاوز إجمالي الفاتورة."); if (!isCredit && paid < totals.total) throw new Error("المبلغ المدفوع أقل من الإجمالي النهائي."); const customer = isCredit ? await requestAsPromise(transaction.objectStore("customers").get(customerId)) : null; if (isCredit && (!customerId || !customer || customer.isDeleted)) throw new Error("اختر عميلًا نشطًا للبيع الآجل.");
  ...
  async createPurchase({ supplierId = "", items, notes = "", paymentType = "نقدي", paidAmount = "", paymentMethod = "نقدي" }) {
    ...
    for (const line of items) {
      ...
      if (pharmacyMode && expiryDate <= dateKey()) throw new Error(`لا يمكن حفظ تشغيلة منتهية أو تنتهي اليوم للمنتج: ${product.name}`);
    ...
    for (const { product, quantity, unitCost, total: itemTotal, packageQuantity, unitsPerPackage, packageCost, packageUnit, salePrice, batchNumber, expiryDate } of resolved) {
      ...
      const batchId = pharmacyMode ? uid("batch") : "";
      ...
      if (pharmacyMode) batches.add({ id: batchId, productId: product.id, productName: product.name, purchaseId: purchase.id, batchNumber, expiryDate, quantity, remainingQuantity: quantity, unitCost, createdAt: date, updatedAt: date });
