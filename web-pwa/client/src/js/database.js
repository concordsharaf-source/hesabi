...
export const db = {
  ...
  async createPurchase({ supplierId = "", items, notes = "", paymentType = "نقدي", paidAmount = "", paymentMethod = "نقدي" }) {
    ...
    for (const { product, quantity, unitCost, total: itemTotal, packageQuantity, unitsPerPackage, packageCost, packageUnit, salePrice, batchNumber, expiryDate } of resolved) {
      ...
      transaction.objectStore("purchaseItems").add({ id: uid("purchase-item"), purchaseId: purchase.id, productId: product.id, productName: product.name, unit: product.unit, quantity, unitCost, total: itemTotal, packageUnit, packageQuantity, unitsPerPackage, packageCost, salePrice, batchNumber, expiryDate, returnedQuantity: 0 });
