...
function dashboardMarkup() {
  ...
  const expiring = isPharmacy() ? state.products.filter((product) => toNumber(product.quantity) > 0 && product.nearestExpiryDate && Math.ceil((new Date(`${product.nearestExpiryDate}T00:00:00`).getTime() - todayAtMidnight) / 86400000) <= 90).sort((a, b) => String(a.nearestExpiryDate).localeCompare(String(b.nearestExpiryDate))).slice(0, 5) : [];
  ...
  <section class="dashboard-split">
    ${isPharmacy() ? `<article class="panel panel--low-stock"><div class="panel__head"><div><span class="eyebrow">تنبيه صيدلي</span><h2>صلاحيات تحتاج متابعة</h2></div><button class="text-button" data-action="navigate" data-view="inventory">عرض المخزون ${icon("arrow", 16)}</button></div>${expiring.length ? `<div class="warning-list">${expiring.map((product) => { const days = Math.ceil((new Date(`${product.nearestExpiryDate}T00:00:00`).getTime() - todayAtMidnight) / 86400000); return `<button class="warning-row" data-action="open-product" data-id="${product.id}"><div class="warning-row__icon">${icon("alert", 18)}</div><div><strong>${escapeHtml(product.name)}</strong><small>تشغيلة ${escapeHtml(product.latestBatchNumber || "—")} · تنتهي ${product.nearestExpiryDate}</small></div><strong class="${days <= 30 ? "is-negative" : ""}">${days < 0 ? "منتهٍ" : `بعد ${amount(days)} يوم`}</strong></button>`; }).join("")}</div>` : `<p class="panel__empty">لا توجد منتجات تنتهي خلال 90 يومًا.</p>`}</article>` : ""}
