...
function setupMarkup() {
  return `<main class="setup-page"><section class="setup-art"><div class="setup-art__brand"><img src="${markImage}" alt="" /><span>حسابي</span></div><div class="setup-art__copy"><p class="eyebrow">المرحلة الأولى</p><h1>دفتر تشغيل متجرك،<br />من أول منتج إلى الفاتورة.</h1><p>البيانات تحفظ محليًا على جهازك لتبقى متاحة حتى دون اتصال بالإنترنت.</p></div><img class="setup-art__image" src="/manus-storage/hesabi-setup-ledger_a7b0fae4.png" alt="رسم تعبيري لأدوات تنظيم المتجر" /></section><section class="setup-form-wrap"><div class="setup-form"><span class="eyebrow">إعداد المتجر</span><h2>لنضبط مساحة عملك</h2><p>أدخل معلومات أساسية تظهر في التطبيق والفواتير.</p><form id="setup-form"><label>اسم المتجر<input name="storeName" required maxlength="60" placeholder="مثال: بقالة الواحة" autofocus /></label><label>نوع النشاط<select name="businessType" required>${BUSINESS_TYPES.map((type) => `<option value="${type}">${type}</option>`).join("")}</select></label><label>العملة<select name="currency" required>${CURRENCIES.map((currency) => `<option value="${currency.code}">${currency.label}</option>`).join("")}</select></label><button class="button button--primary button--wide" type="submit">دخول إلى المتجر ${icon("arrow", 18)}</button></form><small class="offline-note"><span class="presence-dot"></span>يعمل محليًا ويحفظ التغييرات تلقائيًا</small></div></section></main>`;
}
