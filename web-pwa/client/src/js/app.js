...
import { ACCOUNT_ROLES, BUSINESS_TYPES, CURRENCIES, DAILY_EXPENSE_CATEGORIES, DEFAULT_CURRENCY_CODE, EXPENSE_CATEGORIES, MONTHLY_EXPENSE_CATEGORIES, NAV_ITEMS, PACKAGE_UNITS, PAYMENT_METHODS, UNITS } from "./constants.js";
...
async function downloadReportPdf() { try { const file = await createPdfFileFromHtml({ html: reportExportHtml(), filename: `hesabi-report-${dateKey()}.pdf` }); downloadGeneratedFile(file); showToast("تم تصدير تقرير PDF"); } catch (error) { showToast(error.message || "تعذر إنشاء تقرير PDF.", "error"); } }
...
function openPurchaseProductDialog(draft) {
  ...
  overlay.querySelectorAll("[data-dialog-close]").forEach((button) => button.addEventListener("click", closeDialog)); quickForm.addEventListener("submit", async (event) => { event.preventDefault(); try { const values = Object.fromEntries(new FormData(event.currentTarget)); if (await checkBarcode()) return; const math = calculatePackagePurchase(values); if (math.packageQuantity <= 0 || math.unitsPerPackage <= 0) throw new Error("أدخل عدد العبوات والحبات بصورة صحيحة."); const product = await db.createProduct({ ...values, quantity: 0, purchasePrice: math.unitCost, purchasePackageUnit: values.packageUnit, lastPackageCost: math.packageCost }); await refresh(); closeDialog(); draft.lines.push({ productId: product.id, productName: product.name, unit: product.unit, packageUnit: values.packageUnit, salePrice: values.salePrice, ...math }); openPurchaseDialog(draft); } catch (error) { showToast(error.message, "error"); } });
}
