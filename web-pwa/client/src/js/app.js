...
function salesMarkup() {
  ...
  <div class="cart-total"><div><span>الإجمالي المبدئي</span><strong>${money(totals.subtotal)}</strong></div><button class="button button--primary button--wide checkout-launch" data-action="checkout" ${state.cart.length ? "" : "disabled"}>إتمام البيع ${icon("arrow", 18)}</button></div></aside></section>`;
}
