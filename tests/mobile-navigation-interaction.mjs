...
try {
  ...
  await waitFor('[data-action="open-admin-recovery-key"]');
  const recoveryKey = await evaluate(`new Promise((resolve) => { document.querySelector('[data-action="open-admin-recovery-key"]').click(); setTimeout(() => resolve(document.querySelector('#admin-recovery-key-form [name="recoveryKey"]')?.value), 100); })`);
  assert.match(recoveryKey, /^[A-F0-9]{16}$/);
  await evaluate(`(() => { const form = document.querySelector('#admin-recovery-key-form'); form.elements.currentPin.value = '1234'; form.requestSubmit(); })()`);
  ...
  await waitFor('#login-form');
  await evaluate(`document.querySelector('[data-action="open-admin-recovery"]').click()`);
  await waitFor('#admin-recovery-form');
  await evaluate(`(() => { const form = document.querySelector('#admin-recovery-form'); form.elements.username.value = 'admin'; form.elements.recoveryKey.value = ${JSON.stringify(recoveryKey)}; form.elements.pin.value = '5678'; form.elements.pinConfirm.value = '5678'; form.requestSubmit(); })()`);
  await waitFor('[data-bottom-nav] [data-view="inventory"]');
  const recoveredAdmin = await evaluate(`({ title: document.querySelector('.workspace h1')?.textContent?.trim(), recoveryLinkVisible: Boolean(document.querySelector('[data-action="open-admin-recovery"]')) })`);
  assert.deepEqual(recoveredAdmin, { title: "نظرة على يومك", recoveryLinkVisible: false });
