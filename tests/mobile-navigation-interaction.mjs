...
try {
  ...
  const priorProductPurchaseLine = await evaluate(`(() => { const salePrice = document.querySelector('[data-purchase-sale-price="0"]'); const row = salePrice.closest('.purchase-line'); const directPrice = row.querySelector('.purchase-sale-price-field__current'); const inputColor = getComputedStyle(salePrice).color; return { salePrice: salePrice.value, visiblePriceStartsWithValue: directPrice.textContent.trim().startsWith('50'), inputColorVisible: inputColor !== 'rgba(0, 0, 0, 0)' && inputColor !== 'transparent', rowDisplay: getComputedStyle(row).display, fieldCount: row.querySelectorAll('label').length }; })()`);
  assert.deepEqual(priorProductPurchaseLine, { salePrice: '50', visiblePriceStartsWithValue: true, inputColorVisible: true, rowDisplay: 'grid', fieldCount: 7 });
